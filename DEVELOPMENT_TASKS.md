...
- **Documentation:** [x] —
- **Completion Status:** ✅ 2026-07-20 · **Reviewer Notes:** (1) **ملاحظة مسار** (نفس نمط T-062): حقل Files ذكر `public/` لكنه كود ميت غير مخدوم — التعديل الفعلي في `static/app.js` + `static/index.html` + `static/style.css` (المسار المخدوم). (2) `static/index.html`: أُضيف `<script src="/static/js/file_icons.js">` قبل `app.js` مباشرة (app.js يستدعي `FileIcons.getFileIcon` وقت التحميل). (3) `static/app.js`: دالة مشتركة جديدة `fileIconHTML(path)` تستدعي `FileIcons.getFileIcon(path)` وتُخرج `<svg class="file-icon" style="color: var(--icon-*)"><use href="/static/icons/sprite.svg#icon-<id>"></use></svg>` — استُهلكت في 3 نقاط: شجرة الملفات (`renderTreeNode`، بدّلت `getFileIcon(val.ext)` القديمة)، تبويبات المحرر (`renderTabs`، أيقونة لم تكن موجودة أصلًا فأُضيفت)، ورقائق مرفقات الشات (`renderAttachments`، بدّلت الإيموجي `📄` الثابت). الـ mapping القديم المكرر `function getFileIcon(ext)` (امتداد→إيموجي محليًا في app.js) **حُذف بالكامل**. (4) **مسألة النطاق المحسومة**: لا يوجد `@mention` chips ولا diff-panel ولا run-history في الكود الحالي — تحقّقتُ بالبحث الشامل ولا وجود لها؛ يطابق صياغة المهمة "(when they exist)"، فهذه البنود غير قابلة للتطبيق حاليًا (N/A) ولا تشكّل نقصًا. أما `getFileBadgeHTML()` (شارات نشاط الأدوات في الشات: SRC/TREE/DIR + شارات نصية قصيرة لأدوات System بلا امتداد ملف) فهو **نظام منفصل عمدًا وخارج نطاق T-063** — لا يقع تحت tree/tabs/mentions/diff-panel/run-history المذكورة صريحًا في المهمة، ويخدم مفهومًا مختلفًا (شارة نشاط أداة، لا هوية ملف)؛ أُبقي دون تغيير ويُختبَر وجوده صريحًا (`test_get_file_badge_html_is_intentionally_out_of_scope`) لمنع حذفه بالخطأ في مهمة لاحقة. أيقونات المجلدات (📁/📂 في الشجرة) أُبقيت كإيموجي — المهمة تخص "filenames" لا المجلدات. (5) `static/style.css`: قاعدة `.file-icon` جديدة (حجم/flex-shrink فقط، صفر لون خام — اللون يُضبط inline بالتوكن الصحيح per-render، يمر بوابة color-lint). (6) `tests/unit/test_icon_consumption.py` = **8 اختبارات جديدة**: (أ) مصدر وحيد — grep يفحص كل `static/**/*.js` المخدومة بحثًا عن نمط `".ext": "value"` خارج `file_icons.js`، ويؤكد حذف توقيعات mapping الإيموجي القديمة الحرفية (لا فحص إيموجي مفرد لتفادي false-positive مع ⚡ المستخدَم لغرض آخر في action-chip). (ب) تأكيد ترتيب تحميل `file_icons.js` قبل `app.js` في index.html. (ج) "visual snapshot" لشجرة تجريبية 25 مسارًا تغطي كل الأصناف المطلوبة في T-062 — تُنفَّذ عبر استخراج نص دالة `fileIconHTML` الحقيقية من app.js بـ regex وتشغيلها في node الحقيقي (بدون متصفح، بنفس أسلوب `test_file_icons.py`)، مع لقطة (snapshot) ثابتة لكل رمز `#icon-<id>` متوقَّع. `./scripts/check.sh` = **1129 passed, 1 skipped — ALL GREEN** (كان 1121 قبل T-063؛ +8 اختبارات جديدة، صفر تراجع). · **Next Task:** T-064
