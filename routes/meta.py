...
def api_permissions():
    ...
    - أداة localhost-only — **فُحص وحُسم في TSK-737 (القرار 9)**:
      T1 أخطر مسار في نموذج التهديد ⇒ POST مقفل 403 عند التعريض
      الشبكي حتى تحت راية --unsafe-expose-network (أدناه).
