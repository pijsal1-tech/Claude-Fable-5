...
2. **الربط الافتراضي آمن**: `--host 127.0.0.1` (الافتراضي) يقصر
   الوصول على جهازك. **لا تربط على `0.0.0.0` أو عنوان شبكي**
   إلا خلف طبقة مصادقة خارجية (reverse proxy بـ auth، VPN،
   أو نفق SSH) — الربط المكشوف = تنفيذ أوامر عن بُعد بلا قيد.
