...
EMAIL_CLIENTS:
  - mailnesia  → MailnesiaClient      # mailnesia.com (passwordless)
  - mailtm     → MailTmClient          # mail.tm API (sharebot.net)
  - toolpix    → ToolpixMailClient     # mail.tm + toolpix headers
  - vercel     → VercelMailClient      # create-an-email.vercel.app (GET API)

# الـ mix modes:
#   mix  = mailnesia + mailtm + toolpix + vercel
#   tmix = mailtm + toolpix (shortcut)
#   comma-syntax: "mailtm,toolpix,vercel" → random choice 🆕
...
✓ DELAY_BETWEEN يقبل int أو (min, max) tuple للـ random range 🆕
✓ OTP_TIMEOUT (إن كان AUTH_METHOD = OTP)
✓ VERIFY_TIMEOUT (إن كان AUTH_METHOD = Link)
✓ EMAIL_PROVIDER يقبل: اسم واحد | comma-separated "mailtm,toolpix" | mix/tmix 🆕
...
✓ كل Client class فيه:
  ...
  - User-Agent = random_ua() ديناميك لكل session 🆕
  - wait_for_otp(timeout, poll) → Optional[str] [لو OTP]
  - wait_for_link(timeout) → Optional[str] [لو Link]
  - email property
  - info() log في __init__() باسم الـ class (مش في _setup!) 🆕
...
✓ 429 rate limit → retry تلقائي مرة واحدة 🆕

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 [طبقة 4/9] — _get_email_creds() Factory
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✓ دالة _get_email_creds(mail) → dict موجودة (مفصولة DRY — مش inline!) 🆕
✓ لكل Client type: بترجع credentials بـ suffix واضح:
  - MailTmClient / ToolpixMailClient / VercelMailClient → password_mailtm + token_mailtm
  - MailnesiaClient → {} (passwordless)
