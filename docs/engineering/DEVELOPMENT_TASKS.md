...
- **EOP-1** (قرار D-8-أ): حذف `docs/engineering_constitution/` — **مُنفَّذ 🏁
  2026-07-31 بأمر مالك صريح** (13 ملفًا؛ صفر مراجع كود/اختبارات؛ V3 هو
  الدستور الحاكم الوحيد ولا يتأثر).
...
## TSK-725 — P2-3: Workspace Trust — بوابة ثقة عند فتح مجلد [متوسطة] — 🏁 مُقفلة (a+b+c؛ بوابة 2043P/34S)
...
## TSK-726 — P2-4: FI-07 تفكيك app.js [كبيرة — مُفصَّلة S103] 🏁 (a+b+c+d+e مُقفلة — app.js 4204⇒712)
...
- **726a — البنية + أول نقل (الإثبات)** *(تصحيح S103 قبل التنفيذ:
  اكتُشف خطر eval-time — `const CP_ACTIONS = {openFolder, …}` يقيّم
  مراجع الدوال عند التحميل، فملف يسبق app.js يرمي ReferenceError؛
  لذا **ملفات المقاطع الذيلية تُحمَّل بعد app.js** = المكافئ الحرفي
  لترتيب التسلسل الأصلي؛ نقل الثيم يُرجأ لشريحة لاحقة)*: مجلد
  static/js/app/ + حارس (test_app_split.py): (1) ترتيب التحميل =
  UMD-lite ثم app.js ثم app/NN بالترتيب الرقمي؛ (2) كل دوال onclick
  الـ 24 معرَّفة top-level في مجموع الملفات؛ (3) لا ازدواج تعريف
  دالة عبر الملفات؛ (4) node --check لكل ملف. النقل الأول (ذيل
  app.js حرفيًا): Quick Open + Command Palette ⇒ 90_search_palette.js
  وغراء VL + Trust ⇒ 91_vl_trust.js (~390 سطرًا).
...
## TSK-727 — P2-5: غلاف سطح مكتب Windows-أولًا [كبيرة — مُفصَّلة S104] 🏁 (a+b+c منفَّذة — تحقق المالك D-8-ب معلَّق)

### الموازنة التقنية (القرار المعماري — يُسجَّل في DECISION_LOG)
| المعيار | pywebview | Tauri | Electron |
|---|---|---|---|
| توافق المكدس (Flask/Python نقي، بلا bundler) | ✅ أصلي — نفس اللغة | ❌ Rust + Node toolchain | ❌ Node runtime |
| تشغيل الخادم | داخل نفس العملية (خيط) | sidecar معقّد | child-process + IPC |
| حجم التوزيعة | ~15–40MB (PyInstaller) | ~5MB لكن + سلسلة بناء غريبة | 150MB+ |
| البناء على Windows بيد المالك | pip + pyinstaller فقط | rustup + MSVC + npm | npm + electron-builder |
| مخاطرة على قاعدة 2050 اختبارًا | صفر (ملف إطلاق جديد فقط) | متوسطة | متوسطة |

**القرار: pywebview + PyInstaller.** المشروع Python خالص بواجهة
vanilla JS بلا bundler (قرار S103 نفسه) — إدخال سلسلة أدوات Rust/Node
لأجل نافذة WebView يخالف مبدأ أقل التغييرات. وضع المتصفح الحالي
(python server.py) يبقى مسار التشغيل الأول غير المُمَسّ.

### القيود
- بيئة التطوير Linux بلا GUI ⇒ الاختبارات هنا **بنيوية/استيرادية فقط**
  (import-safe، بلا فتح نافذة)؛ التحقق التشغيلي النهائي على Windows
  **بيد المالك حصريًا (D-8-ب)** بقائمة فحص مكتوبة تُسلَّم معه.
- pywebview تبعية **اختيارية** (نمط requirements القائم): غيابها لا يكسر
  أي مسار قائم — desktop.py يفشل برسالة إرشادية واضحة.

### الشرائح
- **727a — الموازنة والقرار**: قيد DECISION_LOG (S104) + ADR في
  ARCHITECTURE_DECISIONS.md (ADR-006: غلاف سطح المكتب) — لا كود.
- **727b — المُطلِق desktop.py**: منفذ حر عشوائي (socket bind 0) ⇒
  تشغيل Flask في خيط خلفي (نفس create_app/app القائم، بلا تعديل
  server.py) ⇒ webview.create_window على العنوان المحلي ⇒ إغلاق
  النافذة يُنهي العملية. حراسة import (غياب pywebview ⇒ رسالة عربية
  + الإحالة لوضع المتصفح). اختبارات بنيوية: الملف import-safe بلا
  pywebview، لا تعديل على server.py، المنفذ الحر يعمل.
- **727c — التغليف وقائمة فحص المالك**: ملف desktop.spec (PyInstaller،
  يضم static/ وthemes/ كـ data) + docs/desktop/WINDOWS_BUILD.md
  (خطوات البناء أمرًا-بأمر) + docs/desktop/OWNER_CHECKLIST.md
  (قائمة فحص D-8-ب: إقلاع، عربية/RTL، ثيمات، WS/بث، فتح مجلد،
  طرفية، إغلاق نظيف) — تُسلَّم للمالك ولا تُقفَل P2 نهائيًا إلا بعد
  تأشيره عليها (يُسجَّل ذلك كبند تحقق خارجي معلَّق لا يمنع متابعة P3).

### معايير القبول
بوابة check.sh خضراء (لا انحدار عن 2050P)؛ desktop.py import-safe؛
صفر تعديل في server.py/app.js؛ الوثيقتان تسلّمان كاملتين.
...
# BATCH-P3 (قرار D-11 — تحت تفويض D-8-ج) — S105

**النطاق** (مصدره PROGRESS §P3): CP-4 hooks، تصليب درز redis (FI-04 مُكيَّفة)،
plugins توسيع، auto-update — أربع مهام: TSK-728..731.

**قرار نطاقي جوهري (D-11)**: FI-04 بصيغتها الكاملة (معمارية worker/عمليات
منفصلة) **تتعارض مع IR-1** (محلي-أولًا، مستخدم واحد، لا worker/process)
ومتطلّبها المسبق FI-01 غير مُنجَز. لذلك تُكيَّف إلى «تصليب درز redis»:
اختبارات عقد + توثيق تفعيل فقط، بلا أي تغيير على مسار الإقلاع الافتراضي.
القيد مسجَّل في DECISION_LOG (صف D-11).

## TSK-728 — CP-4: نظام hooks بعقد «تشديد-فقط» (tighten-only)

**الهدف**: قسم `hooks:` اختياري في config.yaml يسمح للمالك بحقن فحوص
إضافية عند أحداث محددة، بعقد صارم: الـ hook يستطيع **رفع الصرامة فقط**
(منع/تحذير إضافي) ولا يستطيع أبدًا تجاوز ApprovalGate أو إضعافه.

**الأحداث**: `pre_command` (قبل تنفيذ أمر طرفية)، `post_write` (بعد كتابة
ملف)، `post_run` (بعد اكتمال تشغيل).

**العقد الصارم**:
1. فشل hook (استثناء/مهلة) ⇒ **fail-closed**: يُحجَب الإجراء ويُسجَّل تحذير.
2. hook يحاول الإضعاف (السماح بما منعته ApprovalGate) ⇒ يُتجاهَل مع تحذير.
3. غياب قسم `hooks:` ⇒ سلوك اليوم حرفيًا (صفر تغيير سلوكي افتراضي).

**نقاط الحقن المرشّحة** (تُجرَد نهائيًا قبل التنفيذ — D-7):
- chain/agent_loop.py — كتلة verify-step (‏:343–408) ومسار موافقة الأوامر.
- actions/command_runner.py — مسار الموافقة/التنفيذ.
- جرد مواقع نداء ApprovalGate كاملًا (grep) قبل أي شيفرة.
...
**الحالة**: **مغلقة 🏁** (a+b+c منفَّذة — بوابة 2092P/34S).
...
**الهدف**: رفع موثوقية الدرز القائم core/backends_redis.py (درز T-109
الكسول) دون تفعيله افتراضيًا ودون أي معمارية worker.

**الشرائح**:
1. اختبارات عقد تقارن سلوك backend الذاكرة مقابل redis عبر fakeredis
   إن توفّرت وإلا skip موسوم.
2. توثيق تفعيل في دليل FI-12 (خطوات التشغيل اليدوي للمالك).
3. اختبار حارس: الدرز لا يتسرّب إلى مسار الإقلاع الافتراضي
   (server.py يقلع بلا redis إطلاقًا).

**الحالة**: **مغلقة 🏁** (a+b+c منفَّذة — بوابة 2109P/34S خط أساس جديد؛ commits: 4b442bf + 8c91c30).
...
**الحالة**: **مغلقة 🏁** (a+b+c منفَّذة — بوابة 2120P/34S خط أساس جديد؛ commits: 3df2247 + 4b588a9 + 2a72147).
...
## TSK-731 — auto-update ⇒ مُكيَّفة: فحص تحديث يدوي opt-in (لا تحديث تلقائي)

**الحالة**: **مغلقة 🏁** (a+b+c منفَّذة — بوابة 2168P/34S خط أساس جديد؛ commits: 9bfee1c + fe8f1f3 + eb61bf1).

**قرار نطاقي (امتداد روح D-11)**: «auto-update» الكامل (تنزيل/استبدال
exe صامت) يتعارض مع ثلاثة قيود مجرودة: (1) IR-1 محلي-أولًا — لا
phone-home صامت أبدًا؛ (2) شكل التوزيعة النهائي (PyInstaller exe)
معلَّق على تأشير المالك على OWNER_CHECKLIST (727)؛ (3) استبدال
binary ذاتي = سطح أمني جديد كامل (توقيع/تحقق سلامة) بلا عائد الآن.
لذلك تُكيَّف إلى **فحص تحديث يدوي opt-in**: المستخدم يطلب، النظام
يقارن، لا شيء يُنزَّل — قناة التثبيت تبقى يدوية (توثيق) حتى تأشير
المالك.

- **731a — core/update_check.py**: مقارنة إصدارات semver-مبسّطة
  (تدعم لاحقة -rc.N — الإصدار الحالي 1.0.0-rc.1) `compare_versions`
  نقية + `check_for_update(manifest_url, current, timeout=5)` تجلب
  JSON `{"latest": "..", "url": ".."}` عبر requests وترجع
  `UpdateInfo | None` — أي فشل (شبكة/schema/timeout) ⇒ None صامت
  (الفحص لا يكسر شيئًا أبدًا). صفر استيراد وقت-تحميل لـ requests
  (lazy — نمط T-109).
- **731b — نقطة REST عند-الطلب**: `GET /api/update-check`
  (routes/meta.py) — يقرأ قسم config `updates: {check_enabled: false,
  manifest_url: ""}`؛ **الافتراضي معطَّل** ⇒ `{"enabled": false}` بلا
  أي نداء شبكة (حارس اختبار: monkeypatch على requests يرمي — المسار
  الافتراضي لا يلمسه)؛ مفعَّل ⇒ `{enabled, current, latest,
  update_available, url}` مُطهَّرة. **لا polling خلفي إطلاقًا** —
  النداء الوحيد هو طلب المستخدم الصريح.
- **731c — توثيق**: قسم «قناة التحديث» في docs/desktop/WINDOWS_BUILD.md
  — التثبيت يدوي (تنزيل exe جديد واستبدال)؛ ملاحظة صريحة: أتمتة
  التثبيت مؤجلة حتى تأشير المالك (727) واستقرار شكل التوزيعة +
  config.yaml مثال معلَّق.

**معايير القبول**: (1) config افتراضي ⇒ صفر نداء شبكة (اختبار حارس)؛
(2) عقد compare_versions (rc/final/أقدم/أحدث/مساوٍ/نص فاسد⇒None)؛
(3) نقطة REST بعقد مفاتيح + عدم-تسريب (نمط TSK-721)؛ (4) فشل الشبكة
⇒ استجابة سليمة لا استثناء؛ (5) بوابة check.sh خضراء كاملة.

**DAG داخلي**: 731a → 731b → 731c.
...
## TSK-CEV-104 — توسيع معجم أنماط التوجيه العربية (CEV-F-015) [صغيرة]
- **104a — chain/orchestrator.py**: توسيع `_COMPLEX_REQUEST_PATTERNS`
  ليلتقط الصيغ المكافئة المقيسة: «أعد/أعيدي هيكلة» (فعل الأمر —
  مثل r"أعد.{0,6}هيكلة")، «ريفاكتور» (المعرَّبة صوتيًا)، وما يقيس
  مثلها من صيغ فعلية مقابلة للمصادر الموجودة. مراجعة موازية لأنماط
  `_HIGH_RISK_PATTERNS` العربية («تسجيل الدخول» مقابل r"مصادقة").
- **104b — tests/unit/test_routing_matrix.py**: عكس تأكيدات
  `TestLexiconGapF015` (auto_chain→full_chain + matched_signals
  غير فارغة) — الاختباران المثبتان stub هما معيار القبول الجاهز.
- **حارس انحدار**: corpus T-034 (routing_corpus.golden.json) يجب أن
  يبقى أخضر **بلا تعديل golden** — الأنماط الجديدة يجب ألا تغيّر
  درجة أي سيناريو قديم (وإن غيّرت: قرار مالك قبل إعادة التوليد).
...
## TSK-CEV-110 — تسييج دفاعي لمسار السلاسل: نتائج التبعيات + سياق الملفات (يحسم CEV-F-013) [صغيرة]
> **بموجب D-15**: هندسة دفاعية (توسيع NF-18/TSK-404 القائم) —
> ليست نشاط Red Team ولا اختبارًا هجوميًا. تُقفل الحد الموثق في
> ملاحظة نطاق TSK-CEV-109b.
- **110a — تسييج نتائج التبعيات**: `chain/models.py`
  `ChainStep.build_prompt` — الجسم المحقون بعد `[Result from
  {dep_id}]:` يُلف بـ`fence_attached(f"dep_result:{dep_id}", body)`
  (نفس آلية templates.py:48 — تحييد وسم إغلاق مزوّر + تنظيف source).
  العنوان `[Result from …]` يبقى خارج السياج (توجيه للنموذج).
  ينطبق على وضعَي `full` و`summary` (وضع `minimal` بلا محتوى أصلًا).
- **110b — تسييج سياق الملفات المكتشفة**: `chain/context_builder.py`
  `ContextItem.to_prompt_block` — `{self.content}` يُلف
  بـ`fence_attached(f"{self.kind}:{self.source}", content)`.
  العنوان `📄 [kind: source]:` يبقى خارج السياج.
- **110c — تحديث المثبتات وعيًا (بروتوكول AIA-7 حارس 4 / AIA-R8)**:
  التغيير prompt-affecting قصدًا — يستلزم: (1) تحديث goldens
  test_context_policy الحرفية (full/summary/legacy-equivalence:
  اختبار التكافؤ البايتي مع «المنطق القديم قبل T-045» يُعاد صوغه
  ليقارن مع legacy **مسيَّج** — التكافؤ الذي يثبّته صار: نفس
  السلوك عبر السياسات لا نفس بايتات ما قبل NF-18)؛ (2) legacy
  المرجعي في test_map_reduce_dedup (نسبة ≥40% تُعاد قراءتها)؛
  (3) إعادة التقاط `prompt_corpus.golden.json` (capture_corpus.py)
  و`tests/goldens/chain/*.golden.json` (capture_goldens.py) مع
  تصنيف الـdiff يدويًا: **تحسين مقصود** (زيادة سياج فقط — لا حذف
  محتوى ولا تغيير توجيه)؛ (4) توسيع scripts/check_injection_guard.py:
  طبقة رابعة تفحص حضور fence_attached في models.py و
  context_builder.py (قيم حية لا grep) + حذف ملاحظة النطاق البائتة.
- **الحد المتبقي بعد 110 (يوثَّق لا يُنفَّذ)**: حارس
  `INJECTION_GUARD_INSTRUCTION` في system مسار السلاسل يبقى عبر
  الضابط التعويضي (قاعدة «بيانات لا أوامر» 21/21 — تشير حرفيًا
  لوسوم `<attached-content>` فترتبط الآن فعليًا بالمحتوى المسيَّج)؛
  إلحاق الحارس نصيًا بـsystem السلاسل يغيّر 21 لقطة sha256 في
  corpus الأدوار — قرار مالك منفصل إن أراد التوحيد الكامل.
- **معايير القبول**: أسيجة `<attached-content source="dep_result:…">`
  و`source="file:…"` حاضرة في البرومبتات المجمَّعة؛ حارس الحقن
  الموسَّع أخضر (وكسر متعمد = أحمر)؛ goldens المعاد التقاطها diff
  سياج-فقط؛ check.sh ALL GREEN.


## TSK-CEV-111 — توليد fixture `.env` وقت الاختبار بدل تخزينها (يحسم CEV-F-003 نهائيًا) [صغيرة]
**بموجب D-16 (طابور البند 1).** الجذر: Auto-Uploader حذف
`tests/fixtures/sample_project/.env` مرتين (ba2d9f0، 37a371f) رغم
استثناء `.gitignore:11` الصريح — منطق تنظيفه يعامل أي `.env` كسر حقيقي؛
الاستعادة اليدوية بلغت ×36. خيارا F-003 الموثقان: (أ) استثناء بوت —
بيد المالك خارج المستودع؛ (ب) تغيير كود يقفل الثغرة. المنفَّذ: متغيّر
أنظف من (ب) — **التوليد وقت الاختبار**:
- **111a**: في `tests/conftest.py` fixture `sample_project` — بعد
  `copytree` يُكتب `.env` بالمحتوى التاريخي الحرفي (a9f52b5، ترويسة
  «FAKE credentials — NOT real secrets») داخل النسخة المؤقتة فقط؛
  المصدر في الشجرة يبقى بلا `.env` (لا شيء يحذفه البوت).
- **111b**: نفس التوليد في مجمِّعي goldens
  (`tests/goldens/{chain,context}/capture_goldens.py`) بعد `copytree`
  — تكافؤ بيئة الالتقاط مع بيئة الاختبار (وإن كانت goldens الحالية
  لا تُظهر `.env` إطلاقًا: SafeReader/denylist يحجبه — تحقق grep = 0).
- **111c**: إزالة استثناء `.gitignore:9-11` (يعود `.env` محجوبًا كليًا
  — يزيل الالتباس الذي غذّى الإيجابية الكاذبة) + حذف النسخة غير
  المتتبَّعة من الشجرة.
- **الحد الموثق**: تعليمة التوليد تعيش في موضعين (conftest + مجمّعان)
  — ثابت مشترك `SAMPLE_ENV_BODY` في conftest يستورده المجمّعان يمنع
  الانجراف.
- **معايير القبول**: `test_sample_project_fixture_isolated` أخضر بلا
  ملف `.env` في الشجرة؛ goldens replay أخضر بلا إعادة التقاط (صفر
  اعتماد)؛ `git status` نظيف بعد الحذف؛ check.sh ALL GREEN.


## TSK-CEV-112 — طوابير تفويض متعددة المهام (ينفّذ FI-13) [متوسطة]
**بموجب D-16 (طابور البند 5) — ترقية REFERENCE→ACTIVE للانضباط الموثق
في `newskells/.../multi-task-queues.md`.** الفجوة: `grep -in queue`
= صفر في delegate.py/strategies.py — لا وضع متعدد المهام موثوق.
- **112a — وحدة جديدة `chain/delegate_queue.py`**: `QueuedTask`
  (task_id/description/files_context/status/run/carried) +
  `DelegateQueue` يدير DelegateBridge القائم **تتابعيًا حصرًا**
  (multi-task-queues.md:9-25: لا fan-out؛ المهمة التالية لا تُرسل
  إلا بعد هبوط السابقة land):
  - `add_task()` قبل `start()`؛ `start()` يشغّل المهمة الأولى عبر
    `run_delegation` القائم (كل ضمانات T-015/checkpoints تُورَث).
  - **بوابة الموافقة تبقى سيدة**: waiting_approval يوقف الطابور؛
    `land_current()`/`reject_current()` يغلّفان land/reject القائمين
    ثم يقرران التقدّم — **rejected/failed/cancelled = إيقاف الطابور**
    (halted — انضباط «stop and ask»: العمل حول افتراض مكسور ممنوع).
  - **ترحيل القيود المقررة (carry-forward)**: بعد كل هبوط تُستخرج
    حقائق التنفيذ (touched_files + ملخص المنفّذ + ملاحظات الحكم)
    وتُحقن كتلة `[قيود مقررة من مهام سابقة]` في project_context
    للـbrief التالي — البريف يبقى self-contained (السطر :27-31).
  - أحداث WS لكل مهمة: queue_started/queue_task_started/
    queue_task_waiting_approval/queue_task_landed/queue_halted/
    queue_completed (تمر عبر _emit بنمط delegate القائم).
- **112b — اختبارات حتمية (P-11)**: FakeProvider بردود مبرمجة
  ([VERDICT]: APPROVE/REJECT) تثبت: التتابع الصارم (المهمة 2 لا
  provider calls قبل land الأولى)؛ الترحيل (بريف المهمة 2 يحوي كتلة
  القيود بحقائق المهمة 1)؛ الإيقاف عند الرفض؛ اكتمال الطابور؛
  الأحداث بترتيبها.
- **حدود واعية**: لا تغيير في delegate.py نفسه (صفر مساس بعقود
  T-009/T-015 القائمة)؛ لا توجيه آلي للطابور من الـrouter (استهلاك
  لاحق قرار مستقل)؛ التوازي مرفوض بنص المرجع.
- **معايير القبول**: الوحدة الجديدة مغطاة باختبارات حتمية خضراء؛
  صفر تعديل على delegate.py؛ check.sh ALL GREEN.


## TSK-CEV-113 — مهمة تفويض خلفية محكومة (ينفّذ FI-15) [متوسطة]
**بموجب D-16 (طابور البند 7) — الشرط المسبق FI-13 (TSK-CEV-112) مقفل.**
المصدر: CEV-G10 CP-11 (VS Code v1.110 background agents) +
`dispatch-and-poll.md` §Waiting for completion («background and poll…
Trust the working tree and the process state over any progress
display»). الفجوة: مسار التفويض الوحيد يشغَّل من معالج WS بخيط
يموت مع الجلسة منطقيًا — لا حالة قابلة للاستئناف بعد إعادة اتصال
(snapshot) ولا تسليم-وتتبّع hand-off-and-track.
- **113a — وحدة جديدة `chain/background_delegate.py`**:
  `BackgroundDelegateTask` يلف DelegateBridge القائم (صفر تعديل
  عليه — نفس حدود TSK-CEV-112):
  - `start()` يطلق `run_delegation` في خيط daemon ويرجع فورًا
    (hand-off)؛ `is_running/status/task_id` للتتبّع.
  - **حالة آمنة لإعادة الاتصال (reconnect-safe)**: `snapshot()`
    يرجع dict كاملًا بالحالة الراهنة + سجل الأحداث المتراكم
    (تحت قفل) — عميل أعاد الاتصال يستعيد الصورة كاملة من الكائن
    الحي، لا من ذاكرة الجلسة المقطوعة (dispatch-and-poll.md: الثقة
    بحالة العملية لا بآخر إطار عرض).
  - أحداث WS: background_started / background_event (تمرير أحداث
    الجسر كما هي) / background_finished — عبر on_event بنمط
    _emit القائم (ابتلاع مسجَّل).
  - **الثابت الصلب (Non-Goal §15.1 — لا YOLO)**: كل كتابة تبقى
    خلف بوابة الموافقة — الغلاف لا يملك أي مسار land تلقائي؛
    waiting_approval نهائية من منظوره حتى `land()`/`reject()`
    صريحين يغلّفان القائمَين. مسار خلفي يكتب بلا بوابة = مرفوض.
- **113b — اختبارات حتمية (P-11)**: FakeProvider مبرمج + join
  بمهلة تثبت: التسليم الفوري (start يرجع قبل اكتمال الدورة)؛
  الوصول لـwaiting_approval **والبقاء فيها بلا land تلقائي**
  (الثابت الصلب)؛ snapshot بعد «إعادة اتصال» (كائن جديد يقرأ نفس
  المهمة) يحوي الحالة والأحداث كاملة؛ land/reject الصريحان يعملان؛
  فشل المزود = failed بلا استثناء هارب؛ الأحداث بترتيبها.
- **حدود واعية**: صفر تعديل delegate.py وdelegate_queue.py؛ مؤشر
  الواجهة (UI indicator) خارج النطاق — الواجهة الخلفية فقط (استهلاك
  الواجهة قرار منتج لاحق)؛ لا سحابة (no cloud pivot — نص FI-15).
- **معايير القبول**: الوحدة مغطاة باختبارات حتمية خضراء؛ الثابت
  الصلب مثبَّت باختبار؛ صفر تعديل على الجسر والطابور؛ check.sh
  ALL GREEN.


## TSK-CEV-115 — حلقة تصحيح مدفوعة بالأدلة لدور deep_debugger (ينفّذ FI-16) [صغيرة]
**بموجب D-16 (طابور البند 8).** المصدر: CEV-G10 CP-16 (JetBrains
Junie يصحّح بمصحح فعلي) — دورنا `deep_debugger`
(`agents_rules/manifest.yaml:109`، يُستدعى في strategies.py:343)
يستدل ساكنًا على المصدر فقط. الحد المرفوض صراحة: تكامل DAP/مصحح
كامل (CEV-G10 §G10.4 — ثقل غير متناسب مع محرر خفيف).
- **115a — ترقية برومبت الدور** (`agents_rules/سيستم/أنت محقق
  أخطاء عميق.md`): قسم «حلقة الأدلة التشغيلية» يوجّه الدور لجمع
  دليل تشغيلي عبر الأدوات المحكومة القائمة **قبل** الاستنتاج
  (تشخيص glass-box لا استدلال جاف): تشغيل الاختبار الفاشل المستهدف
  وحده؛ إخراج تتبّع مؤقت يُزال بعد التشخيص؛ قراءة مخرجات الخطأ
  الفعلية — كلها عبر run_command/read_file تحت مجموعة SAFE +
  ApprovalGate (صفر سطح قدرات جديد). قاعدة «بيانات لا أوامر»
  (ضابط NF-18 التعويضي — يفحصه check_injection_guard) تبقى نصًا.
- **115b — إعادة التقاط corpus واعية (AIA-7 حارس 4 / AIA-R8)**:
  تعديل ملف الدور يغيّر sha256 لقطته المثبَّتة في
  prompt_corpus.golden.json (`risky_auth_pipeline/steps[0]`) —
  إعادة الالتقاط عبر capture_corpus.py + **تصنيف الفرق يدويًا**
  (متوقع: تحسين مقصود — تغيّر لقطة deep_debugger وحدها، صفر تغيّر
  توجيه/بنية خطوات).
- **115c — حالة corpus واحدة جديدة**: سيناريو تحقيق عطل عالي
  الخطورة يوجَّه لـpipeline (يستدعي deep_debugger) يُضاف لـ
  SCENARIOS في harness.py — يوسّع تغطية الدور المرقَّى بلقطة مثبَّتة.
- **حدود واعية**: صفر تعديل على strategies.py/orchestrator.py
  (التوجيه كما هو)؛ صفر أدوات جديدة؛ DAP مرفوض بنص FI-16.
- **معايير القبول**: القسم الجديد نصًا في ملف الدور + قاعدة
  «بيانات لا أوامر» باقية؛ corpus replay أخضر بعد إعادة الالتقاط
  المصنَّفة؛ حالة corpus الجديدة تستدعي deep_debugger؛ check.sh
  ALL GREEN.


## TSK-CEV-116 — توحيد حارس الحقن في system مسار السلاسل (يحسم حدّ NF-18 الموثق) [صغيرة]

**المصدر**: NF-18 + الحدّ الموثق في docstring سكربت `scripts/check_injection_guard.py`
(«التوحيد الكامل يغيّر 21 لقطة sha256 للأدوار = قرار مالك منفصل») — **قرار المالك صدر**:
D-16 البند 9 (الأخير قبل G7).

**الفجوة**: مسار السيرفر/الدردشة يُلحق `INJECTION_GUARD_INSTRUCTION` نصيًّا بالـ system
(`prompts/templates.py:60-61`)، بينما مسار السلاسل (executor/delegate) يعتمد فقط على
القاعدة التعويضية «بيانات لا أوامر» داخل ملفات الأدوار — حماية أضعف وغير موحّدة.

**قرار نقطة الحقن — عند مواقع النداء لا داخل AgentLoader**:
- `tests/unit/test_agent_manifest.py` يثبّت `prompt.content` بمساواة تامة (‎:279/:322/:329/:336) —
  حقن داخل `AgentLoader.load` يكسرها ويلوّث كل مستهلكي `AgentPrompt` (منهم تسجيل الجودة).
- لذلك: دالة مساعدة تُطبَّق عند تركيب طلب المزوّد فقط؛ `AgentPrompt.content` يبقى نقيًّا.

**ملاحظة تفويض**: تعديل `chain/delegate.py` هنا **مأذون** — قيد «صفر تعديل» في
TSK-CEV-112/113 كان حدّ نطاق تلكما المهمتين لا تجميدًا دائمًا، وهذا بند مالك صريح يستهدف
هذه المواقع تحديدًا. حراس المصدر في اختبارات الطابور/الخلفية يمنعون فقط سلاسل
`delegate_queue`/`BackgroundDelegateTask` — إضافة استيراد `guarded_system` آمنة.

### 116a — الدالة المساعدة
- في `prompts/templates.py`: `guarded_system(content: str) -> str` تعيد
  `content + "\n\n" + INJECTION_GUARD_INSTRUCTION` (ومع content فارغ تعيد الحارس وحده)
  — نفس الفاصل المستخدم في SYSTEM_PROMPT/CORE_SYSTEM_PROMPT حرفيًّا.

### 116b — توصيل أربعة مواقع + ترقية سكربت الحارس
- `chain/executor.py::_call_provider` (‎:441): `system_prompt=guarded_system(agent_prompt.content)`.
- `chain/delegate.py` المواقع الثلاثة (planner ‎:315-318، executor ‎:344-347، code_reviewer ‎:384-387):
  `system = guarded_system(agent_prompt.content)` (والفرع الفارغ يبقى "" — لا حارس بلا دور).
- `scripts/check_injection_guard.py`: قسم جديد يؤكد استخدام `guarded_system` في الملفين؛
  تحديث docstring لإغلاق نص «الحد المتبقي الموثق»؛ إبقاء فحص القاعدة التعويضية 21/21.

### 116c — تحديث harness + إعادة التقاط واعية (AIA-R8)
- `tests/goldens/prompts/harness.py::run_scenario`: قياس sha256/len على
  `guarded_system(agent_prompt.content)` (النص المركّب الفعلي المُرسل للمزوّد) — يغيّر
  **كل** لقطات الخطوات الـ28.
- بروتوكول: نسخ الذهبي إلى ‎/tmp → `python -m tests.goldens.prompts.capture_corpus` →
  فرق برمجي صارم: صفر تغيير استراتيجية/عدد خطوات/user_prompt، ولكل خطوة
  `new_len - old_len == len("\n\n" + INJECTION_GUARD_INSTRUCTION)` ⇒ تصنيف تحسين مقصود.

### 116d — اختبار وحدة (سياج)
- `tests/unit/test_injection_guard_unification.py`: عبر `FakeProvider` (يسجّل
  `RecordedCall.system_prompt`): دورة تفويض كاملة تثبت أن **الثلاثة** system_prompts
  المسجّلة تنتهي بالحارس؛ ومسار executor كذلك؛ و`guarded_system` نفسها (فاصل + strip).

**البوابة**: `bash scripts/check.sh` أخضر بالكامل (شاملًا إعادة الالتقاط) + سكربت الحارس المرقّى.

## TSK-CEV-114 — حراس التلاعب بالاختبارات في مراجعة التفويض (ينفّذ FI-14) [صغيرة]
**بموجب D-16 (طابور البند 6) — الرقم 113 محجوز لـ FI-15 في الـDAG،
فالمهمة تأخذ 114 وعيًا لا سهوًا.** المصدر:
`newskells/.../review-and-land.md` §«Check the tests before trusting
the gates» (:8-19). الفجوة: معايير الحكم في
`chain/prompts/delegate_review.md` لا تذكر التلاعب بالاختبارات —
مراجع قد «يمتص» تعديلات صامتة على اختبارات قائمة لأنها تُخضّر البوابة.
- **114a — سطور معايير في `delegate_review.md`**: قسم فرعي تحت
  «معايير المراجعة» يعتبر كلًا مما يلي **تغيير عقد (contract change)
  ⇒ REWORK أو REJECT — لا يُمتص صامتًا أبدًا**:
  1. تعديل غير مُكلَّف به على اختبارات قائمة (unbriefed edits) —
     الـbrief لم يطلبه ⇒ علّمه، لا تعتمده.
  2. إضافة skip/disable/تعليق لاختبار ⇒ عامل الاختبار المعطَّل
     كأنه فاشل (treat as failing).
  3. تليين تأكيدات (exact→contains/truthy، توسيع نوع الخطأ،
     توسيع tolerance) ⇒ نفس المعاملة.
- **114b — اختبار تسييج واحد**: يثبت أن نص المعايير يبقى في
  الـprompt المُحمَّل فعليًا عبر `_load_prompt("delegate_review.md")`
  (نفس مسار delegate.py:229) — تحرير مستقبلي يُسقط المعايير يكسر
  الاختبار (FI-14: «one fencing test asserting the criteria text
  survives prompt edits»).
- **فحص تثبيت مُسبق (منجز)**: delegate_review.md غير مثبَّت بايتيًا
  في prompt corpus golden ولا goldens/chain ولا check_injection_guard
  — صفر إعادة التقاط مطلوبة.
- **حدود واعية**: لا تعديل على delegate.py (مثبَّت بـTSK-CEV-112)؛
  لا تغيير على صيغة الحكم `[VERDICT]:` (regex :706 يعتمد عليها)؛
  رفض DAP/أدوات جديدة — سطور معايير + اختبار فقط (تكلفة FI-14
  المنصوصة).
- **معايير القبول**: المعايير الثلاثة نصًا في الـprompt؛ اختبار
  التسييج أخضر؛ صفر تعديل delegate.py؛ check.sh ALL GREEN.
...
## TSK-737 — التعريض الشبكي: نموذج التهديد + الحارس fail-closed (القرار 9 من تسلسل D-19 — الأخير)

### السياق الحاكم
- **قرار المالك D-19 (البند 9)**: «التعريض الشبكي **آخرًا حصرًا** —
  "يغيّر نموذج التهديد بالكامل، وأي ميزة تُبنى قبله ستحتاج غالبًا
  إلى مراجعة أمنية إضافية"» — كل القرارات 2–8 مُغلقة 🏁؛ هذا آخر
  قرارات التسلسل.
- **ثلاثة تعهدات مرحَّلة يجب الوفاء بها هنا** (نصوص ملزمة في
  المواصفات والكود):
  - (أ) TSK-734: «localhost-only — مراجعة القرار 9 الأمنية يجب أن
    تعيد فحص هذا المسار» (core/permissions_overrides.py:20 +
    routes/meta.py POST /api/permissions).
  - (ب) TSK-735: «تحرير المفاتيح من UI قرار منتج لاحق وبعد القرار 9
    حصرًا» (core/provider_keys.py:26-28 — لا دالة كتابة).
  - (جـ) TSK-736: «القرار 9 يجب أن يعيد فحص مسارات ACP
    (الكتابة/الإذن)» (CHANGELOG §TSK-736 حدود واعية) — نوع WS
    `acp_prompt` يشغّل subprocess بأمر من config.
- **الوضع القائم**: `--host` افتراضيه `127.0.0.1` (server.py:2589)؛
  `app.run(host=args.host, ...)` (server.py:2920) يقبل أي عنوان بلا
  أي حارس؛ README §حدود النشر يحذّر نصيًا فقط («لا تربط على 0.0.0.0
  إلا خلف طبقة مصادقة خارجية») — **تحذير توثيقي بلا إنفاذ كودي**.

### نموذج التهديد (المخرج الأول للقرار 9)
**الأصل المهدَّد**: من يصل للمنفذ يملك المشروع والطرفية (README بند 1
— لا مصادقة على REST/WS إطلاقًا). عند ربط غير loopback يصبح كل نظير
شبكي «المستخدم» بكامل صلاحياته:

| # | المسار | القدرة عند التعريض | الخلاصة |
|---|---|---|---|
| T1 | `POST /api/permissions` (TSK-734) | مهاجم شبكي يقلب `force_command_approval=false` ويوسّع `agent.command_allowlist` ⇒ **RCE كامل** عبر /api/run التالي | أخطر مسار — تعطيله عند التعريض إلزامي |
| T2 | `provider_keys.json` (TSK-735) | لا مسار كتابة REST (قرار واعٍ قائم) وعقد عدم-الترديد يمنع تسريب القيمة في أي استجابة؛ لكن مهاجمًا شبكيًا **يستهلك** مفاتيح المالك المدفوعة عبر تبديل المزود + prompts | العقد القائم يصمد؛ الاستنزاف يُحسم بحسم T-عام |
| T3 | WS `acp_prompt` (TSK-736) | مهاجم شبكي يطلق عملية وكيل معرَّفة في config ويقودها بـ prompts؛ الكتابات خلف ApprovalGate لكن انظر T4 | تعطيله عند التعريض إلزامي |
| T4 | **قناة الموافقة نفسها هي WS غير المُصادَق** | المهاجم الشبكي المتصل بالـ WS **يوافق على أفعاله بنفسه** — ApprovalGate التفاعلية توفّر **صفر** حماية ضد نظير شبكي | **الحكم الحاسم**: لا قيمة أمنية لأي تقسية سلوكية طالما القناة مفتوحة ⇒ الحل الوحيد السليم منع الربط المكشوف نفسه |
| T5 | بقية الأسطح (/api/run، /api/write، /api/restore، الجلسات، الذاكرة) | تنفيذ/كتابة/قراءة كاملة — بنفس منطق T4 لا تُقسَّى فرادى | يغطيها الحارس العام |
| T6 | CSRF من متصفح على جهاز المالك نفسه (تهديد localhost قائم) | endpoints الـ JSON تتطلب `Content-Type: application/json` ⇒ preflight يفشل بلا CORS headers (Flask لا يرسلها) — **دفاع ضمني قائم** | يُوثَّق كملاحظة؛ لا عمل |

**الحكم المعماري (من T4)**: المصادقة لا تُلحَق بهذا المنتج ترقيعًا —
أداة localhost لمستخدم واحد (README + عقد CEV «عقد localhost»).
**المسار السليم الوحيد للوصول عن بُعد قائم اليوم بلا أي تغيير**: نفق
SSH / VPN / reverse-proxy بمصادقة — كلها تنتهي إلى loopback على جهاز
الخادم، فتعمل مع الافتراضي `127.0.0.1` كما هو.

### القرارات الواعية الملزِمة
1. **لا مصادقة تُبنى (REJECT نطاق)**: authn/authz كاملة = إعادة
   تصميم منتج (خارج R0.1)؛ الوصول عن بُعد = طبقة خارجية تنتهي إلى
   loopback (تعمل الآن بلا تعديل). القرار 9 يُنفَّذ **تحصينًا
   للحدود** لا فتحًا لها.
2. **حارس إقلاع fail-closed**: `--host` غير loopback ⇒ **رفض
   الإقلاع** برسالة شارحة (تذكر النفق/الـ proxy والراية)، إلا مع
   راية صريحة جديدة `--unsafe-expose-network` اسمها يصرّح بالخطر
   (opt-in واعٍ — سابقة force_command_approval افتراضه الآمن).
   تغيير سلوك مقصود بقرار مالك D-19-9 (يُوثَّق في CHANGELOG).
3. **حتى تحت الراية — المسارات الثلاثة المرحَّلة تُقفَل**:
   (أ) `POST /api/permissions` ⇒ 403 برسالة «localhost فقط» (GET
   يبقى — قراءة glass-box)؛ (جـ) WS `acp_prompt` ⇒ `acp_error`
   رفضًا (لا إطلاق subprocess بقيادة نظير شبكي)؛
   (ب) provider_keys يبقى كما هو (لا مسار كتابة أصلًا + عقد
   عدم-الترديد قائم ومُختبَر) — والقرار القائم «تحرير المفاتيح من
   UI بعد القرار 9» يُحسم الآن: **يبقى مؤجَّلًا بلا أجل** (لا يُبنى
   في TSK-737 — T4 يجعل أي مسار كتابة أسرار عبر قناة غير مُصادَقة
   خطأً تصميميًا؛ يُسجَّل FI إن طلبه المالك).
4. **defense-in-depth تحت الراية**: `_force_command_approval()`
   يعيد True قسرًا عند التعريض (يعلو على overrides وعلى false
   الصريحة — نفس نمط علو Workspace Trust :356)؛ **مع توثيق صريح
   لحدوده**: لا يحمي من نظير WS (T4) — قيمته ضد وصول REST-فقط.
5. **الحكم بواقعة الربط لا بعنوان الطالب**: الحارس يُقيَّم من
   `host` المُمرَّر للإقلاع (واقعة زمن-إقلاع ثابتة) لا من
   `remote_addr` لكل طلب (قابل للتضليل خلف proxy — X-Forwarded
   ألاعيب خارجة). الوصول: دالة `_network_exposed()` تقرأ
   `ctx.config["host"]` (كاتبه الوحيد main:2881 بافتراض argparse
   `127.0.0.1`؛ غيابه = سياق اختبار/قبل-main = محلي) — دالة لا حالة
   وحدة متحوّلة (امتثال lint_handler_state).
6. **loopback = تعريف صريح مُختبَر**: `127.0.0.0/8` كاملة + `::1`
   + `localhost` (حرفيًا، case-insensitive). كل ما عداه — بما فيه
   `0.0.0.0` و`::` وأسماء المضيفين وعناوين LAN — **مكشوف**
   (fail-closed: ما لم يثبت أنه loopback فهو مكشوف).
7. **صفر تغيير في السطحين المجمّدين**: لا مسار REST جديد ولا نوع
   WS جديد (36 REST / 34 WS كما هما)؛ صفر شبكة/subprocess في
   الاختبارات (الحارس يُختبَر بحقن config/args لا بربط فعلي).

### الشرائح
- **737a — الوحدة النقية** (`core/network_guard.py`):
  `is_loopback_host(host: str) -> bool` (تطبيع + ipaddress لنطاق
  127/8 و::1 + حرفية localhost؛ أي فشل تحليل ⇒ False أي مكشوف) +
  `exposure_refusal_message(host)` (نص رسالة الرفض الموحّد — يذكر
  النفق والراية). اختبارات وحدة بارامترية: 127.0.0.1/127.1.2.3/
  ::1/localhost/LOCALHOST ⇒ آمن؛ 0.0.0.0/::/192.168.x/عنوان عام/
  hostname/فارغ/قمامة ⇒ مكشوف.
- **737b — توصيل الحارس** (server.py + routes/meta.py):
  حارس الإقلاع في main (رفض + SystemExit(2) برسالة الوحدة ما لم
  تُمرَّر `--unsafe-expose-network`) + `_network_exposed()` +
  قسر `_force_command_approval()` + قفل POST /api/permissions
  (403) + رفض `_ws_acp_prompt` (acp_error). اختبارات تكامل بحقن
  ctx.config: التعريض يقلب القسر ويقفل POST ويرفض acp_prompt؛
  الوضع المحلي (غياب المفتاح أو 127.0.0.1) صفر تغيير سلوك —
  انحدار كامل أخضر.
- **737c — إغلاق التوثيق**: README §حدود النشر (الحارس الكودي بدل
  التحذير النصي + جدول نموذج التهديد المختصر + مسار النفق الموصى)
  + تحديث تعليقات «القرار 9 سيعيد الفحص» في الملفات الثلاثة إلى
  «فُحص وحُسم في TSK-737» + CHANGELOG §TSK-737 + PROGRESS (إغلاق
  تسلسل D-19 كاملًا 🏁).

### حدود واعية
- لا مصادقة، لا HTTPS/TLS، لا rate-limiting — كلها لوازم «خدمة
  ويب» والمنتج ليس كذلك (عقد localhost CEV).
- تحرير provider_keys من UI: مؤجَّل بلا أجل (القرار الواعي 3-ب).
- خادم ACP / registry وكلاء عن بُعد: يبقيان REJECT (TSK-736 حدود).
- T6 (CSRF): الدفاع الضمني القائم يُوثَّق فقط — لا عمل.

### معايير القبول
- إقلاع `--host 0.0.0.0` بلا راية ⇒ رفض برسالة شارحة (exit 2)؛
  مع `--unsafe-expose-network` ⇒ يعمل والمسارات الثلاثة مقسّاة.
- إقلاع افتراضي (127.0.0.1) ⇒ سلوك اليوم حرفيًا (صفر تغيير).
- تحت التعريض: POST /api/permissions=403، acp_prompt=acp_error،
  force_command_approval=True قسرًا؛ GET /api/permissions يعمل.
- السطحان المجمّدان بلا تغيير (36/34)؛ check.sh ALL GREEN.

**DAG (TSK-737)**: 737a → 737b → 737c.

---

## TSK-738 — تنفيذ القرارين المستقلين 10/11 (بموجب D-21) — housekeeping + عدة قياس

**السياق الحاكم**: D-19 وصفهما «مستقلان — لا يغيّران سلوك المنتج»؛
D-21 رسّخ نصّيهما وفوَّض التنفيذ. **قيد صارم: صفر تغيير في سلوك
المحرر** — لا مساس بأي كود إنتاجي (core/ chain/ routes/ server.py...)
ولا بالسطحين المجمّدين (36 REST / 34 WS) ولا بأي اختبار قائم.

### الشريحة 738a — القرار 10: مزامنة فرع genspark_ai_developer
- **الواقع المقيس**: الفرع 0 أمام main / 154 خلفه؛ رأسه 2fe610d
  (2026-08-01) = عمل توثيقي قديم دُمج مضمونه سلفًا.
- **القرار الواعي (الاختيار المفوَّض)**: **مزامنة fast-forward إلى
  main** لا حذف — المسوغات: (1) الفرع 0-أمام ⇒ المزامنة لا تفقد شيئًا
  والحذف لا يكسب شيئًا إضافيًا؛ (2) أدوات بيئة التطوير (بروتوكول
  GenSpark) قد تفترض وجود الفرع — حذفه قد يكسر تشغيلًا مستقبليًا؛
  (3) المزامنة تحقق عين مقصد المالك «منع العمل بالخطأ على كود قديم»
  (الفرع يصبح مطابقًا لـ main بايت-بايت)؛ (4) غير هدّامة ومتسقة مع
  روح append-only. التنفيذ: `git push origin main:genspark_ai_developer`
  (fast-forward نقي — يُرفض تلقائيًا لو لم يكن كذلك = أمان مدمج).
- **قبول**: `git ls-remote` يُظهر الفرعين على نفس SHA.

### الشريحة 738b — القرار 11: عدة قياس CEV-F-006 للجهاز الحقيقي
- **حد واعٍ**: تشغيل القياس على جهاز المالك **فعل مالك** — لا يمكن
  تنفيذه من هذه البيئة. مُخرَجنا = **عدة قياس جاهزة + خط أساس Sandbox
  مرجعي + دليل تشغيل** ليقارن المالك مباشرة.
- **`scripts/perf_probe.py`** (جديد — خارج بوابتي mypy/flake8 للنطاق
  الإنتاجي لكنه يُكتب نظيفًا): يقيس بلا أي خادم شبكي:
  (1) زمن استيراد server.py (بديل «زمن فتح المحرر» القابل للقياس آليًا)؛
  (2) بناء مستودع اصطناعي 5k ملف (نفس مواصفة fixture `big_project`
  في test_search_perf.py حرفيًا: 50×100، الإبرة في pkg25/mod_050) ثم
  قياس مساري البحث (search_project + tool_search_code) **بنفس عقد
  أفضل-من-3** المستخدم في الاختبار (TSK-CEV-119)؛
  (3) لقطة CPU/RAM (nproc + /proc/meminfo + resource.getrusage).
  المخرَج: تقرير نصي + JSON اختياري (--json). ملاحظة FPS: واجهة
  المحرر HTML/JS في متصفح المالك — قياس FPS يدوي عبر DevTools
  (يوثَّق في الدليل، لا يُدّعى قياسه آليًا — نفس روح P-11).
- **خط أساس Sandbox**: تشغيل العدة هنا (2 vCPU / ~1GB) وترسيخ الأرقام
  في دليل التشغيل ليقارن المالك رقمًا-برقم.
- **دليل التشغيل**: قسم جديد في docs/deployment_threat_model.md؟ لا —
  الأنسب ملف مستقل صغير `docs/perf_probe_runbook.md` (عربي): خطوات
  التشغيل على Windows/Linux، جدول خط أساس Sandbox، عتبات الحكم
  (اختفت ⇒ بيئية / ظهرت ⇒ حقيقية تُفتح لها TSK)، وقياس FPS اليدوي.
- **قبول**: العدة تعمل هنا وتنتج تقريرًا؛ البوابة الكاملة ALL GREEN
  بلا أي تعديل اختبار.

### الشريحة 738c — الإغلاق الوثائقي
CHANGELOG §TSK-738 + PROGRESS (ترويسة + قيد تكملة 12) — القراران
10/11 مُغلقان 🏁 (11 يُغلق من جهتنا بتسليم العدة؛ نتيجة القياس على
جهاز المالك تُلحق عند ورودها وقد تفتح TSK إصلاح إن ظهرت الرجفة).

**DAG**: 738a → 738b → 738c (738a مستقلة فعليًا لكن تُنفَّذ أولًا لبساطتها).
