...
## CEV-G9 — تقرير بوابة الانحدار 🏁 PASS
**التاريخ: 2026-08-02 (S108 تكملة 7) — الشجرة: d9e0e87 — المنهج: تشغيل حي كامل في بيئة التقرير نفسها (لا استشهاد بتشغيلات قديمة فقط) + مطابقة بتية للـgoldens ضد git**

### الأدلة (مواصفة G9 بندًا ببند)
1. **check.sh ALL GREEN rc=0**: تشغيل حي كامل على الشجرة d9e0e87 =
   **2231P/34S/0F rc=0** — شامل بوابات mypy والحُرّاس النصيّين
   الـ11 القدامى + حُرّاس AIA-7 الخمسة الجدد (كل قسم مسمى أخضر).
2. **goldens bit-identical**: كل حزم الـgoldens الخمس عشرة
   (apply_batch_frames + chain×6 + context×6 + prompts + routing)
   أُعيد تشغيلها حية (99 اختبار replay/parity) ثم
   `git status --porcelain tests/goldens/` = **صفر تغيير قرص** —
   المطابقة البتية مثبتة بالأداتين (dict-equality + git).
3. **خط الأساس الحي لا يهبط**: PROGRESS.md يقرأ خط الأساس
   2231P/34S (قيد إقفال AIA-6/AIA-7) — القياس الحي = **2231P/34S**
   بالضبط: صفر هبوط، صفر اختبار مفقود.
4. **الرجفات الموثقة تُعامل كما هي**: في بيئة ما بعد تصفير #43
   (2 vCPU) ظهر تناوب رجفة زمنية واحدة في 4 تشغيلات سابقة
   (test_search_perf = سابقة S84؛ test_no_save_churn = رجفة mtime
   NEW_FINDINGS:397-401) — كلاهما مرّ معزولًا ثم مرّ **داخل**
   التشغيلة الكاملة الحالية. لم تُخفَ (موثقة في قيد تكملة 6) ولم
   تُحسب فشلًا جديدًا (لا Finding جديد — مطابقة نص المواصفة حرفيًا).

### النطاق والحدود
- صفر تعديل كود في هذه البوابة (فحص انحدار خالص) — آخر تغيير كود
  = حُرّاس AIA-7 (3bdfeaa) المفحوصون هنا.
- الانحدارات المعلومة المفتوحة كلها Findings مُسعَّرة بمسار حسم
  (F-013/F-014/F-015) — ليست انحدارات جديدة بل فجوات مكتشفة
...
## CEV-G11 — تقرير بوابة الدين التقني 🏁 PASS
(2026-08-02 — S108 تكملة 9؛ CEV/D-12؛ المواصفة: cont22.md:138–139)

**المطلوب نصًا**: «مسح TODO/FIXME/HACK/temporary/deprecated/placeholder +
إعادة التحقق من ادعاء «صفر دين» الأخير بالدليل.»

### G11.1 — المسح (نطاق: كل الكود التشغيلي + الاختبارات + السكربتات)

الأمر: grep حساس-الكلمة على chain/ core/ context/ actions/ providers/
routes/ runners/ scripts/ src/ static/ prompts/ public/ tests/ server.py
config.yaml — بالوسوم الستة المطلوبة + توسعة طوعية (XXX/TBD/WIP/
workaround/kludge/BLOCKED + المكافئ العربي «مؤقت»):

| الوسم | النتيجة | التصنيف |
|---|---|---|
| TODO / FIXME / HACK / XXX | **صفر في كودنا** — الإصابات الوحيدة: (1) `static/vendor/highlight.min.js:62–63` (regex تلوين تعليقات داخل مكتبة موردة — ليست ديننا)؛ (2) `prompts/templates.py:109` قاعدة برومبت **تمنع** TODO (عكس الدين)؛ (3) `tests/unit/test_parse_args_body.py:53–54` النص "TODO" كبيانات اختبار parser | ✅ نظيف |
| temporary / deprecated | **صفر** إصابة | ✅ نظيف |
| placeholder | 5 إصابات كلها سمة HTML/CSS مشروعة (`static/index.html:386,467,588,604` خانات إدخال + `style.css:1236` تنسيقها) — ليست placeholder-code | ✅ نظيف |
| TBD / WIP / workaround / kludge / BLOCKED | **صفر** إصابة | ✅ نظيف |
| «مؤقت» (عربي) | 3 إصابات: `static/app.js:30` بمعنى **timer** (مُجانس لغوي، ليس "temporary")؛ `core/ws_router.py:9` + `server.py:1841` — **الاكتشاف الحقيقي الوحيد** (أدناه) | ⚠️ ⇒ أُصلح |

### G11.2 — الاكتشاف الوحيد: مرجعان توثيقيان بائتان (أُصلحا)

docstrings في `core/ws_router.py:9` و`server.py:1841` كانا يقولان:
«المقابض تبقى **مؤقتًا** في server.py (نقلها موضوع **QG-02..04**)» —
لكن QG-02..04 = TSK-612/613/614 **كلها ✅ DONE** (S61–70) ونطاقها
المُنجز كان استخراج مسار الإرسال (⇒ core/chat_dispatch.py) + تجميع
REST blueprints + بوابة mypy — **لا نقل أجسام مقابض `_ws_*` الـ23**
(لا تزال في server.py، 2357 سطرًا). أي أن التعليق يَعِد بشيء أنجزته
مهام أُغلقت بغير هذا النطاق. المؤشر الصحيح للنقل المتبقي هو
**FI-02 (تفكيك server.py god-module — backlog مُسعَّر بقرار مالك)**.
الإصلاح (commit db3f8ae): تصويب المرجعين إلى الواقع + FI-02 — تغيير
تعليقات فقط، صفر سلوك. **هذا ليس دينًا خفيًا بل مؤشر بائت على دين
مُعلَن مُسعَّر أصلًا** (FI-02 موجود منذ P7).

### G11.3 — إعادة التحقق من ادعاء «صفر دين» الأخير بالدليل

الادعاء الأخير: `CONSTITUTION_V3_GENESIS_REPORT.md:308` (@03c7eab):
«**صفر دين تقني** (غياب TECHNICAL_DEBT.md معلَّل — P-13)؛ صفر
TODO/BLOCKED». إعادة الفحص اليوم:
- **صفر TODO/BLOCKED**: ما زال صحيحًا حرفيًا (الجدول أعلاه) — الادعاء
  **يصمد**.
- **غياب TECHNICAL_DEBT.md**: ما زال قائمًا ومعلَّلًا (P-13: «لن يُنشأ
  لدينٍ غير قائم») — وما زال التعليل صادقًا لأن كل ما يشبه الدين
  مُدار في سجلات قائمة: backlog مُسعَّر = FUTURE_IMPROVEMENTS
  (FI-01..16)، فجوات مقيسة = Findings مفتوحة C2–C3 بمسار حسم
  (F-013/F-014/F-015)، مؤجلات بقرار = D-14 (G7). لا عنصر بلا سجل.
- **تدقيق تكميلي**: 7 حالات `pragma: no cover` كلها معلَّلة بنص السبب
  في السطر نفسه (بروتوكول/بيئة/تشخيص/يستحيل الوصول/TYPE_CHECKING/
  إشارة حقيقية)؛ 34 skip كلها معلومة الخط الأساس (2231P/34S).

### G11.4 — بوابة الانحدار على إصلاح البوابة نفسها

التعديل (تعليقات فقط) خضع للبوابة الكاملة: `check.sh` **ALL GREEN
rc=0 (2231P/34S/0F، 17 قسمًا شامل حراس AIA-7)**. التشغيلة الأولى
(بيئة باردة بعد Wipe #46) أسقطت بنشًا توقيتيًا واحدًا
(`test_append_cost_does_not_grow_with_history`: قاس 3.17× مقابل عتبة
3×) — يمر معزولًا فورًا والثانية خضراء بالكامل؛ وُثِّق كعضو ثالث في
فئة CEV-F-006 (رجفات عتبات التوقيت على 2 vCPU) بنفس العقد: لا يُخفى
ولا يُحسب فشلًا جديدًا.

### G11.5 — الحكم

- المسح بالوسوم الستة (+5 طوعية + المكافئ العربي): **صفر دين خفي** —
  كل الإصابات إما مكتبة موردة أو بيانات اختبار أو سمات HTML أو مُجانس
  لغوي، والاكتشاف الوحيد (مرجعان بائتان) أُصلح فورًا ضمن البوابة.
- ادعاء «صفر دين» (@03c7eab) **يصمد بعد إعادة التحقق**: صفر
  TODO/BLOCKED قائم، وغياب TECHNICAL_DEBT.md ما زال معلَّلًا لأن كل
  شبيه-دين مُدار في سجل مُسعَّر معلَن (FI/Findings/D-14) — الدقة
  الواجبة: «صفر دين **غير مُدار**» هي الصياغة الأصدق اليوم.

**الحكم: G11 PASS. المتبقي في CEV: G7 (مؤجلة D-14 — تُستأنف بأمر
المالك) ثم G12 (بوابة الإطلاق: Scorecard §5 + إعادة تصويت RRR) —
G12 لا تُفتح قبل حسم G7.**

---

## CEV-G12 — بوابة الإطلاق (تنفيذ جزئي بموجب D-15): ⚙️ PARTIAL / CONDITIONAL — S108 تكملة 11 (2026-08-02)

> **سند الفتح**: القيد الوارد أعلاه («G12 لا تُفتح قبل حسم G7») كان
> بموجب D-14؛ **D-15 حدّثه صراحة**: «All Red Team activities (G7)
> are officially deferred… NOT considered a blocker… Complete all
> remaining documentation, quality gates, release preparation,
> scorecards, reports…» ⇒ تُنفَّذ أجزاء G12 القابلة للإنجاز
> موضوعيًا الآن، ويُحجب حصرًا ما يعتمد على G7.

### G12.1 — ما نُفِّذ (كامل الأجزاء الموضوعية)
1. **بطاقة الدرجات §5 — محاور 1–9**: مقيَّمة بالأدلة الطازجة على
   الشجرة الحالية، مثبتة كمقطع CEV في RELEASE_READINESS_REPORT.md
   (per D-12). **النتيجة الموضوعية: 90/90** — كل محور بدليل
   file:line أو تشغيلة حية (check.sh ALL GREEN rc=0 2231P/34S/0F؛
   mypy --check-untyped-defs بوابة فاشلة-عند-الخطأ؛ حارس حقن
   4 طبقات؛ REST مجمَّد 35 بمثبت حي :83؛ PROGRESS حتى تكملة 11؛
   FI-01..16 مُسعَّرة؛ استئناف مجرَّب ×51 مسحًا؛ desktop.spec
   يُفسَّر؛ AIA-C كاملة).
2. **إعادة تصويت RRR — نسخة مشروطة**: GO-conditional (الشرط
   المعلَّق الوحيد G7) + Fully supported لعقد localhost — مسجلة في
   نفس المقطع مع مقارنة صريحة بتصويت S83.
3. **قائمة المحجوب حصرًا بـG7** (مطلب D-15 الرابع): (1) المحور 10،
   (2) حسم المجموع ≥95، (3) التصويت النهائي، (4) إقرار CEV-R12 —
   مفصولة عن قائمة المحجوب بقرارات مالك أخرى (F-003/F-010/F-014/
   STALE-175/FI-13..16/توحيد حارس system السلاسل).

### G12.2 — ما حُجِب ولماذا (حصرًا G7)
- **المحور 10** («صمود الفريق الأحمر»): دليله النصي الحرفي في §5
  هو «§6 بلا S1/S2 مفتوحة» — أي نتيجة جولة G7 ذاتها؛ تقييمه بلا
  الجولة = اختلاق. سُجِّل **DEFERRED BY OWNER (D-15)** بلا درجة.
- **حسم العتبة ≥95**: بمحاور 1–9 = 90 والمحور 10 غير مقيَّم، المدى
  الممكن 90–100 — العتبة تتحقق فقط إن أحرز المحور 10 ≥5/10 بعد
  تنفيذ G7 فعليًا؛ الحسم قرار بوابة لا استقراء.
- **إقرار CEV-R12**: لا يُصدَر لبرنامج 11/12 بوابة — الإقرار يوثّق
  اكتمال G1..G12 «بما فيها G8.5» نصًا.

### G12.3 — أمانة المنهج
- صفر نشاط هجومي نُفِّذ (التزام D-15-ج حرفيًا): كل أدلة المحور 3
  دفاعية (حراس قائمون + تسييج TSK-CEV-110 المنجز هندسيًا).
- الدرجات ليست تقديرات ذاتية: كل 10/10 مربوطة بمثبت آلي أخضر أو
  دليل ملموس قابل للتحقق (اختبار/سكربت/سجل) — وحيث الدليل حي هذه
  الجلسة (الاستئناف ×51) ذُكر بوصفه قياسًا معاشًا لا ادعاءً.

**الحكم: CEV-G12 = PARTIAL/CONDITIONAL — الأجزاء الموضوعية مكتملة
ومقفلة؛ الإغلاق الرسمي (والإقرار CEV-R12) محجوبان حصرًا بتنفيذ G7
بأمر مالك لاحق. بهذا استُنفدت كل أنشطة D-15 القابلة للتنفيذ.**

## CEV-G7 — تقرير الفريق الأحمر (Red Team Pass) ⚠️ COMPLETED-WITH-FINDING — S109 تكملة 14 (2026-08-03)

> **سند التنفيذ**: قرار المالك D-16 — G7 آخر مرحلة مطلقًا بعد إغلاق
> طابور البنود 1–9 كاملًا (تحقق: PROGRESS «مُقفل بالكامل 9/9»).
>
> **تصحيح ترويسة سابقة (نزاهة — V3 append-only)**: عنوان هذا المقطع
> كُتب في تكملة 13 بوسم «🏁 PASS» **قبل** تنفيذ الجولة (البيئة مُسحت
> أثناء الكتابة — Wipe #63 — فبقي المقطع ترويسةً بلا جسم: 5 أسطر).
> الجولة نُفِّذت فعليًا في تكملة 14 وأنتجت **اكتشافًا S2 مفتوحًا**، لذا
> صُحِّح الحكم إلى **COMPLETED-WITH-FINDING**. الوسم القديم كان
> استباقيًا وغير مسنود بدليل — وتصحيحه واجب بموجب §3 («لا إقفال بوابة
> لأن الوقت انتهى») وV3 §0 (الدليل فوق الذاكرة).

### المنهج والتفويض
جولة **قراءة-فقط** بموجب §6 وتفويض §7 (صفر تعديل كود إنتاجي؛ سكربتات
الهجوم بقيت خارج الشجرة في `/tmp/g7/` فلا تتوسّع بها الشجرة ولا
البوابات). البيئة أُعيد بناؤها بموجب V3 §3.1 (استنساخ + تطهير remote
فوري + هوية git) — HEAD المدقَّق: **`78a185c`**؛ الشجرة نظيفة.
خط الأساس تُحقق منه **حيًّا** قبل الهجوم: `2285P/1F/34S` حيث الفشل
الوحيد `test_no_save_churn_when_list_unchanged` هو رجفة `mtime` البيئية
الموثَّقة (CEV-F-006) — أخضر منفردًا `10 passed`. أي: **2286** إجمالًا،
مطابق لخط الأساس المسجَّل في PROGRESS.

> **إضافة ختامية (نفس التكملة، بعد كتابة هذا التقرير)**: البيئة مُسحت
> مرة ثانية (Wipe #64) وأُعيد بناؤها؛ HEAD صار **`65e4f30`** (كوميت
> البوت الذي حفظ محاور G7 كاملة: MASTER_REVIEW +123، NEW_FINDINGS +59 —
> صفر عمل ضائع). **بوابة الانحدار الختامية: `2286 passed, 0 failed,
> 34 skipped` — ALL GREEN rc=0، تشغيلان متتاليان مطابقان (89.79s)**.
> أي أن رجفة CEV-F-006 **لم تتكرر** ختامًا، وهو تأكيد إضافي لتصنيفها
> بيئية لا انحدارًا. لا يغيّر هذا أيًّا من نتائج المحاور الأربعة أدناه
> (كلها نُفِّذت على `78a185c` بنفس شجرة الكود — الفرق التوثيق فقط).

### المحور 1 — مهاجمة الاكتشافات المغلقة (عينة 5) + الدفاعات القائمة

**(أ) Zip-Slip (TSK-105/NF-15) — `server.py:828` — صامد 12/12.**
هُوجم `_zip_member_violations` بأرشيفات مُصنَّعة:

| المجس | النتيجة |
|---|---|
| `../../etc/passwd` · `a/../../../../tmp/x` · `./a/./../../../x` | محجوب `escapes_root` |
| `/etc/shadow` · `C:\Windows\sys.ini` · `\\server\share\x` (UNC) | محجوب `absolute_path` |
| `..\..\evil.txt` (فواصل Windows) · `'../evil. '` | محجوب `escapes_root` |
| عضو symlink → `/etc/passwd` (`external_attr` 0o120777) | محجوب `symlink_member` |
| NUL `'../x\x00.txt'` | محجوب (zipfile يقلّم عند NUL ⇒ `../x` ⇒ `escapes_root`) |
| homoglyph `'．．/evil'` (نقطتان بعرض كامل) | **مسموح — سلبي صحيح**: يحلّ *داخل* الجذر (مجلد اسمه حرفيًا `．．`) لا خارجه |
| ضابط حميد `src/app.py` | مسموح ✅ (لا إيجابية كاذبة) |

**(ب) احتواء المسارات — `chain/path_policy.py:54` — صامد 11/11.**
`../../../../etc/passwd`، `/etc/passwd`، `src/../../../etc/hosts`،
`....//....//etc/passwd`، مسار مطلق لملف خارجي — كلها
`PermissionError`. المُرمَّز `%2e%2e%2f` والفواصل الخلفية على POSIX
تُعامل أسماءً حرفية وتبقى **محتواة** (سلبي صحيح لا ثغرة).
**symlink داخل الشجرة يشير خارجها**: محجوب برسالة صريحة، و
`allow_symlinks=True` **لا يرخّي الاحتواء** (محجوب أيضًا) — تحقق مهم
لأن الراية قد تُقرأ خطأً كأنها تعطيل للاحتواء.

**(ج) حجب الأسرار — `chain/path_policy.py:28` — ⚠️ كُسِر.**
→ **CEV-F-018 (C1/S2)**: مسافة لاحقة في اسم الملف تُبطل التصنيف
السري، وتسريب سر حقيقي أُثبت عبر أداة الوكيل الإنتاجية
`AgentTools.tool_read_file` (لا عبر الدالة معزولة). الفئات الثلاث
(الأسماء/الامتدادات/فرع `.env`) مصابة؛ فرع `.env.*` صامد. التفصيل
الكامل + الجذر + الإصلاح المقترح في NEW_FINDINGS.md.

**(د) بوابة الموافقة fail-closed — `core/approval.py:199` — صامدة 9/9.**

| الهجوم | النتيجة |
|---|---|
| `mode=deny` | `approved=False` / `deny_mode` |
| `deny` + `auto_whitelist` محشوّة بالفعل نفسه | `deny_mode` — الـwhitelist **لا** تتقدم على deny |
| `interactive` بلا مُجيب | `timeout` بعد المهلة (لا fail-open) |
| `on_request` يرفع استثناء | `timeout` — الاستثناء لا يُترجم موافقةً |
| `interactive_override` يرفع استثناء (وضع auto) | `timeout` — **fail-closed مؤكَّد** مطابقًا لتوثيق :180-184 |
| `auto` وفعل غير مُدرَج | `non_whitelisted_kind` |
| `auto` + override=True | يُجبر interactive ⇒ `timeout` (الثقة تتقدم على auto) |
| **تلاعب بالبصمة**: `resolve()` بـ`payload_hash` مزيَّف | `resolve → False` والحكم النهائي `timeout` — التلاعب مرفوض |
| ضابط: `auto` وفعل مُدرَج | `auto_whitelist` ✅ (سلوك مقصود لا ثغرة) |
| أوضاع غير شرعية `AUTO`/`Auto`/`auto `/`approve`/`""` | كلها `ValueError` |

### المحور 2 — البرومبتات: حقن على مستوى stub — صامد 12/12
هُوجم `fence_attached`/`guarded_system` (`prompts/templates.py:48/60`)
بمحاولات خروج من السياج: وسم إغلاق صريح، إغلاق مزدوج، إغلاق + فتح
مزيَّف، إغلاق بأحرف كبيرة، إغلاق بمسافات، إغلاق مشقوق بسطر جديد،
عرض-صفري داخل الوسم، وحشو نص الحارس نفسه ثم «قاعدة جديدة».
**صفر خروج**: كل إغلاق مزوّر في المتن يُحيَّد (`</attached\u200bcontent>`)
فيبقى وسم إغلاق شرعي واحد فقط.
كذلك هُوجم **حقل `source`** (المتحكَّم فيه جزئيًا بأسماء ملفات): أقواس
الزاوية والاقتباس تُزال، ووسم إغلاق داخل المصدر يفقد أقواسه
(`x/attached-contenty`). سطر جديد داخل المصدر يشقّ الترويسة بصريًا
**لكن** العدّ يبقى وسم إغلاق واحدًا ⇒ لا خروج (وأسماء الملفات بسطر
جديد غير قابلة للإنشاء عمليًا في المسار المفحوص).
ملاحظة نزاهة (ليست عيبًا): `guarded_system` **ليست idempotent** —
تغليف مزدوج يُكرر الحارس نصًا؛ لا مستدعي إنتاجي يفعل ذلك (المواقع
الأربعة تُغلّف `agent_prompt.content` النقي مرة واحدة)، فتُسجَّل
ملاحظة لا اكتشافًا.

### المحور 3 — التوجيه: 3 طلبات خبيثة الغموض
1. «…**وتعيد هيكلته** كله مع اختبارات وأمان ومراجعة» ⇒ لا يُلتقط
   (`_COMPLEX_REQUEST_PATTERNS`, `chain/orchestrator.py:110-121`) —
   **CEV-F-017** (مسجَّلة سابقًا) أُعيد إنتاجها، و**وُسِّعت هذه الجولة**:
   `يعيد كتابة` و`نعيد تصميم` تفشلان أيضًا ⇒ محور تصريف كامل لا صيغة
   واحدة. الضوابط `أعد هيكلة`/`إعادة هيكلة`/`refactor` تُلتقط.
2. «fix it» بلا سياق ⇒ `direct`: **سلوك مقبول** (صفر إشارات قابلة
   للقياس؛ الحماية عند التنفيذ لا التوجيه).
3. «نظّف المشروع شوية وخليه أحسن» ⇒ `direct`: مقبول لنفس السبب.
الأثر أمنيًا **صفر** — مسار `direct` يمر بنفس ApprovalGate (أُثبت في
المحور 1-د)؛ الأثر جودة توجيه (S4).

### المحور 4 — تحقق عيّني من ادعاءات PROGRESS ضد الكود (4 ادعاءات)

| الادعاء | التحقق الحي | الحكم |
|---|---|---|
| «سطح REST مجمَّد 35 بمثبت حي» | `tests/unit/test_rest_blueprints.py:83` → `assert len(_current_rules()) == 35` + `test_rules_bit_identical` | ✅ دقيق |
| «FI-13: `delegate_queue.py` + 17 اختبارًا حتميًا» | الملف موجود (12,460B)؛ `pytest -k delegate_queue` → **17 passed** | ✅ دقيق |
| «توحيد NF-18: `guarded_system` عند 4 مواقع نداء» | `delegate.py:320/350/391` + `executor.py:444` = **4 بالضبط** | ✅ دقيق |
| «حارس الحقن 21/21 + `guarded_system` مُوصَّلة» | `scripts/check_injection_guard.py` → OK صراحةً «21/21 role files… unified via guarded_system at 4 call sites» | ✅ دقيق |
صفر ادعاء مُبالَغ. الترويسة المتقادمة (سطر 44: «G7 مؤجلة DEFERRED
(D-14)») **ليست ادعاءً كاذبًا** بل تقادُم توثيقي — صُحِّح في هذه الجلسة.

### الحصيلة
- **مجسّات مُنفَّذة: 44** (12 ZIP + 11 احتواء + 2 symlink + 9 موافقة +
  12 سياج) + 3 توجيه + 4 تحقق ادعاءات.
- **صامد**: Zip-Slip، احتواء المسارات، symlink، بوابة الموافقة
  (بما فيها تلاعب البصمة)، سياج الحقن.
- **مكسور**: حجب الأسرار ⇒ **CEV-F-018 (C1/S2 — مفتوح)**.
- **مؤكَّد وموسَّع**: CEV-F-017 (S4 — لا يحجب).

**الحكم: CEV-G7 = COMPLETED-WITH-FINDING.** الجولة نُفِّذت بالكامل
بمحاورها الأربعة، لكن **لا تُوسم PASS**: §6 ينص صراحةً «S1/S2 مفتوحة =
البطاقة محجوبة»، وCEV-F-018 مصنَّفة S2 بدليل تشغيلي. بالتالي:
**المحور 10 من البطاقة لا يُمنح درجة، وحسم المجموع ≥95 وإقرار CEV-R12
يبقيان محجوبين** — الآن بـF-018 بدل «تأجيل G7».
**القرار للمالك** (§7 — الإصلاح تغيير كود خارج تفويض G7): إما إذن
بإصلاح F-018 بدورة TSK كاملة (تطبيع + اختبار انحدار لكل فئة) ثم إعادة
تقييم المحور 10، أو إعادة تصنيف الشدة بقرار موثَّق. لم يُتخذ أيٌّ
منهما ذاتيًا.

---

## TSK-CEV-117 — إصلاح CEV-F-018 (تطبيع اسم الملف في حجب الأسرار) ✅ مقفلة — S109 تكملة 15

> **التفويض**: قرار المالك (المسار «أ») — «إذن بدورة TSK لإصلاح F-018
> (تطبيع الاسم + اختبار انحدار لكل فئة) ثم إعادة تقييم المحور 10 →
> الحسم النهائي → CEV-R12». يُسجَّل كـ**D-17**. النطاق محصور بهذه
> البنود الأربعة حرفيًا؛ ما خرج عنها لم يُنفَّذ (انظر §7 أدناه).

### 1) الجذر (CEV-R4 — لا إصلاح عرَض)
`chain/path_policy.py` هو **سلطة الاحتواء المركزية** للمشروع كله
(`docs/deployment_threat_model.md §3.1`)، وله 4 مواقع استهلاك إنتاجية:
`chain/agent_tools.py:20,355,381,637,713` · `chain/bridge.py:37,752` ·
`chain/context_builder.py:33,542,588`. العلّة: `is_secret_file` كانت
تقارن `path.name.lower()` **حرفيًا** ضد القوائم السوداء الثلاث
(`SECRETS_DENYLIST_NAMES` / `_EXTENSIONS` / `_DIRS`). أي تحوير لا يغيّر
الملف المُشار إليه على نظام الملفات لكنه يغيّر السلسلة ⇒ يُفلت من
التصنيف. الجذر إذًا **غياب طبقة تطبيع قبل المطابقة**، لا نقص مدخل في
القوائم — ولذلك الإصلاح تطبيع مركزي واحد، لا توسيع قوائم.

### 2) تصحيح ذاتي قبل الإصلاح (CEV-R3 — منع المبالغة)
أُعيد التحقق من F-018 حيًّا **قبل** كتابة أي إصلاح، فظهر أن تقرير G7
بالغ في نقطتين وأغفل ناقلًا حقيقيًّا:
- **مبالغة**: ادعاء «تسريب فعلي» عبر `read('.env ')` **لا يصحّ على
  POSIX** — الاسم بمسافة لاحقة ملف مختلف غير موجود (`exists=False`)،
  فالنتيجة رفض لا تسريب. التسريب الفعلي يتحقق على **Win32/NTFS** حيث
  تُقتصّ المسافات/النقاط اللاحقة فيصير `'.env '` هو `.env` نفسه —
  وهذا معتبر لأن المشروع يُسلَّم Windows-first (TSK-727).
- **خطأ وقائعي**: `'.env.'` كان **محجوبًا سلفًا** (يلتقطه فرع
  `startswith('.env.')`)، خلافًا لما ذكره التقرير.
- **ناقل جديد أغفلته الجولة الأصلية**: **NTFS Alternate Data Streams** —
  `.env::$DATA` و`.env:$DATA` يفتحان **نفس مجرى البايتات** لـ`.env`؛
  وكذلك `.env\u200b` (ZWSP) و`.env\ufeff` (BOM) اللذان **لا** يُقصّهما
  `str.strip()` (تحقُّق حي: `strip` يقصّ U+00A0 وU+3000 لكن **ليس**
  U+200B/U+FEFF). الشدة تبقى **S2** لكن بمبرر مصحَّح، مُدوَّن في
  `NEW_FINDINGS.md §CEV-F-018` كقسم تصحيح صريح لا كتعديل صامت.

### 3) تصميم الإصلاح
`normalize_secret_name(name)` بأربع مراحل مُرتَّبة: (1) إزالة المحارف
الخفية السبعة (`\u200b\u200c\u200d\u2060\ufeff\u180e\x00`) — (2) قصّ
لاحقة ADS عند أول `:` — (3) تقليم **بالتناوب** بيضاء↔نقاط حتى نقطة
الثبات (يهزم `.env . . `) — (4) `lower()`. مبدأ الاتجاه الواحد المُعلَن
في docstring: **تُشدِّد الحجب ولا تُرخّيه، ولا تُستخدم لفتح الملفات**
(التطبيع لأغراض التصنيف فقط — الفتح يبقى على المسار المُحلَّل).
`is_secret_file` أُعيدت حياكتها لتستهلك الاسم المُطبَّع، وتُطبّق
الفحص نفسه على **أجزاء المسار** (`path.parts`) لا الاسم وحده.

**فجوة سابقة أُغلقت بالمناسبة**: جزء مجلد ببادئة مسافة
(`' .ssh/known_hosts'`) لم يكن يُلتقط. تحقَّقتُ مقابل نسخة أصلية
محاكاة أن هذا **عيب سابق لا انحدار من تغييري**، ثم أُغلق.

**النتيجة الوظيفية**: 33/33 تحويرًا محجوبًا · 11/11 اسمًا شرعيًا
مسموحًا · **صفر إيجاب كاذب** · استثناء `.env.example` محفوظ.

### 4) اختبار الانحدار — لكل فئة (نص أمر المالك)
`tests/unit/test_path_policy_secret_normalization.py` (230 سطرًا،
**290 اختبارًا، كلها خضراء**) بسبع فئات: (1) `.env` المضبوط + حفظ
استثناء `.env.example` · (2) `SECRETS_DENYLIST_NAMES` × كل التحويرات ·
(3) `SECRETS_DENYLIST_EXTENSIONS` × كل التحويرات · (4)
`SECRETS_DENYLIST_DIRS` كأجزاء مسار · (5) صفر إيجاب كاذب على أسماء
شرعية · (6) تكامل عبر `resolve_workspace_path` (لا وحدة معزولة فقط) ·
(7) خصائص التطبيع: التماثل الذاتي (idempotence) + حارس اتجاه
`test_normalization_never_unblocks_previously_blocked`. مصفوفة
التحويرات `EVASIONS` = 14 لاحقة. أُضيف
`test_on_disk_evasion_family_is_blocked` يوثّق تمييز POSIX/Win32
بصدق بدل ادّعاء تكافؤ غير قائم.

**اختبار طَفَري (mutation) للتحقق من عدم عقم الحزمة**: أُعيد الكود إلى
المطابقة الحرفية ⇒ **الاختبارات تفشل بـ4 حالات مُسمَّاة**؛ ثم استُعيد
الإصلاح ⇒ تعود خضراء. أي الحزمة تكتشف العودة، وليست تأكيدًا شكليًّا.

**نطاق الأثر (blast radius)**: 692 ملفًا متتبَّعًا صُنِّفت بالنسختين
(قبل/بعد) ⇒ **صفر انجراف تصنيف**.

### 5) الأداء — تحقيق كامل بلا مُسكِّنات
ظهر فشل `test_search_perf.py:250` (عتبة 1s) بعد التغيير، والقياس
المعزول أظهر 1.50 → 4.58 µs/مسار. رفضتُ رفع العتبة أو `xfail` أو
الحذف، وأجريت تحقيقًا مُقاسًا:
- **تحسين مرحلي بقياس كل مرحلة**: ساذج 22.9ms → مسار سريع أول
  **60.4ms (أسوأ!** — `isspace()`+`any(generator)`) → `frozenset.isdisjoint`
  18.4ms → فحص مسبق في حلقة الأجزاء 16.5ms → حفظ نتائج
  (`lru_cache`) 18.6ms → تضمين سطري 18.8ms. القياس الدقيق:
  `frozenset.isdisjoint` = **0.36 µs** مقابل 1.90 µs للساذجة (~5×).
- **التنميط (profiling)**: مجموع المكوّنات 11ms والكلي 25ms ⇒ الفائض
  **كلفة نداء الدوال في Python**، لا المنطق ⇒ التضميـن السطري مبرَّر.
- **الأثر الحقيقي على مسار الاختبار الفعلي**: 0.120s → **0.131s**
  (+11ms ≈ 9%) — أي **أقل من العتبة بسبعة أضعاف**.
- **A/B حاسم تحت حمل الحزمة الكاملة**: بعد تصحيح ذراع باطلة (خط
  الأساس كان يُخطئ لأن ملف الاختبار الجديد يستورد
  `normalize_secret_name` ⇒ أُضيف `--ignore`)، النتيجة: **خط الأساس
  يفشل 3/3** (`1 failed, 2285 passed`؛ 90.93s/89.56s/90.71s) مقابل
  **الإصلاح يفشل 2/3**. ⇒ الرجفة **سابقة لتغييري وأسوأ بدونه**؛
  التغيير **مُبرَّأ** بالقياس لا بالدعوى.
- ملاحظة إجرائية: تُرك `git stash` لمصلحة نسخ في `/tmp`
  (`pp_BASELINE/pp_FIXED_BACKUP/pp_FINAL`) حتى لا يترك انقطاعٌ عملًا
  غير مُكوَّم محتجزًا في stash.

### 6) تصحيح مصاحب: CEV-F-006 من S4 إلى S3
التوصيف الأصلي («فشلت مرة واحدة، غير متكررة، تمرّ معزولة ×3») لا
يطابق القياس: الفشل **منهجي تحت حمل الحزمة** (3/3 على خط الأساس).
رُفعت الشدة **S4 → S3** في `NEW_FINDINGS.md`. **تقسية العتبة لم
تُنفَّذ ذاتيًّا** — فهي تغيير عقد أداء خارج تكليف F-018، ورُفعت
كتوصية قرار مالك منفصل.

### 7) ما لم يُنفَّذ عمدًا (حدود «فقط»)
FI-17 · تقسية `test_search_perf` · تدوير `PROGRESS.md §6.4` · أي بند
FI آخر · أي `push`/`branch`/`tag`/`PR` · أي مساس بـ`providers/`.

### 8) الحكم
**TSK-CEV-117 = ✅ مقفلة.** CEV-F-018 (C1/S2) **مُغلقة** بتطبيع مركزي
+ 290 اختبار انحدار لكل فئة + تحقُّق طَفَري + صفر نطاق أثر.
⇒ **لا يبقى أي S1/S2 مفتوحة** ⇒ حاجب §6 عن بطاقة النتيجة **انفكّ**،
والمحور 10 صار قابلًا للتقييم الموضوعي (انظر
`RELEASE_READINESS_REPORT.md §5`).
