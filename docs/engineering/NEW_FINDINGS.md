...
### FI-16 — منفَّذ (S109 تكملة 13، بموجب D-16 طابور البند 8 — TSK-CEV-115)
**حلقة التصحيح المدفوعة بالأدلة لدور deep_debugger حاضرة.** المصدر:
CEV-G10 CP-16 (Junie يصحّح بمصحح فعلي) — الدور كان يستدل ساكنًا
فقط. المنفَّذ: (115a) قسم «حلقة الأدلة التشغيلية (قبل الاستنتاج)»
في `agents_rules/سيستم/أنت محقق أخطاء عميق.md` — ثلاث تعليمات جمع
دليل تشغيلي عبر الأدوات المحكومة القائمة (SAFE + ApprovalGate —
صفر سطح قدرات جديد): تشغيل الاختبار الفاشل المستهدف وحده؛ قراءة
مخرجات الخطأ الفعلية؛ تتبّع مؤقت يُزال بعد التشخيص + قاعدة تنزيل
الثقة عند تعذر الدليل (فرضية ساكنة لا سبب جذري مثبت). قاعدة
«بيانات لا أوامر» (ضابط NF-18) باقية نصًا — check_injection_guard
أخضر 21/21. (115b) إعادة التقاط corpus واعية (AIA-7 حارس 4/AIA-R8)
بتصنيف برمجي صارم: الخطوة المتغيرة الوحيدة
risky_auth_pipeline/pl_scout (deep_debugger) في sha256+len فقط —
**صفر تغيير توجيه/بنية خطوات في كل السيناريوهات** ⇒ تحسين مقصود.
(115c) سيناريو corpus جديد `crash_investigation_pipeline` (تحقيق
عطل تزامن/فساد بيانات عالي الخطورة) يتوجه pipeline ويستدعي
...
### CEV-F-017 — فجوة معجمية متبقية: صيغة المضارع «تعيد هيكلته» لا تُلتقط في معجم التعقيد (G7 Red Team)
**C4 / S4.** أثناء جولة G7 (مجسات توجيه خبيثة الغموض): الطلب العربي
«…وتعيد هيكلته كله مع اختبارات وأمان ومراجعة» يتوجه `direct`
(request_complexity=0.0) رغم أنه إعادة هيكلة كاملة + اختبارات +
أمان + مراجعة. الجذر: أنماط `_COMPLEX_REQUEST_PATTERNS`
(orchestrator.py:110-115) تلتقط «إعادة…هيكلة» و«أعيد/اعيد هيكلة»
لكن **ليس تاء المضارعة** («تعيد هيكلته») — إثبات: regex بديل
`[أات]ع(?:د|يد\w{0,2})\s*.{0,6}هيكل` يلتقطها. امتداد لنفس فئة
CEV-F-015 (حُسمت جزئيًا بـTSK-CEV-104) — صيغة صرفية إضافية.
**ليست ثغرة أمنية**: مسار direct يمر بنفس بوابات الموافقة
(ApprovalGate) — الأثر جودة توجيه فقط (نية معقدة تهبط طبقة أدنى).
مجسّا G7 الآخران («fix it» بلا سياق، وطلب تنظيف غامض واسع) توجّها
direct وهو سلوك مقبول (لا إشارات قابلة للقياس — والحماية عند
التنفيذ لا التوجيه). **الأولوية: منخفضة (S4)** — مرشح FI لاحق
(توسيع المعجم للصيغ الصرفية + إعادة التقاط corpus واعية)؛ لا يحجب
بطاقة النتيجة (ليس S1/S2).

### CEV-F-018 — حجب الأسرار يُتجاوَز بمِحارف بيضاء لاحقة في اسم الملف (G7 Red Team)
**C1 / S2 — مُثبت بالتشغيل الحي (لا استنباط) @ 78a185c (2026-08-03).**
أثناء جولة G7 (محور «حاول كسر الدفاعات القائمة — حجب الأسرار»):
`is_secret_file` (chain/path_policy.py:28-52) يقارن `path.name.lower()`
حرفيًا بلا **تطبيع للمحارف البيضاء/النقاط اللاحقة**، فأي اسم بمسافة
لاحقة يُصنَّف «غير سري» ويمر من البوابة.

**الدليل التنفيذي (عبر أداة الوكيل الإنتاجية `AgentTools.tool_read_file`،
لا عبر الدالة معزولة)**:
```
read('.env')  -> ❌ خطأ: Access denied: '.env' matches blocked secret patterns.
read('.env ') -> API_KEY=sk-LIVE-SECRET-DO-NOT-LEAK      ← تسريب فعلي
```
كل الصيغ التالية تُسرّب أيضًا عبر نفس المسار: `'.env  '`، `'.env\t'`،
`'./.env '`، `'sub/../.env '` (التطبيع يزيل `..` ويُبقي المسافة).

**نصف قطر الانفجار — الفئات الثلاث كلها مصابة** (لا `.env` فقط):
| الفئة | مثال محجوب | صيغة تمر |
|---|---|---|
| `SECRETS_DENYLIST_NAMES` (:17-20) | `id_rsa` ✅ | `id_rsa ` ❌ / `id_rsa.` ❌ |
| `SECRETS_DENYLIST_EXTENSIONS` (:21-23) | `cert.pem` ✅ | `cert.pem ` ❌ |
| فرع `.env` (:33-36) | `.env` ✅ | `.env ` ❌ |
استثناء موثَّق: فرع `.env.*` (`startswith`) يبقى صامدًا
(`.env.local ` = محجوب ✅) — لذا الثغرة في المطابقة **التامة** حصرًا.

**الجذر (CEV-R4)**: غياب تطبيع اسم قبل المطابقة — وليس نقصًا في
قوائم الحجب. `path_policy` هي «الدلالة المرجعية للاحتواء في المشروع
كله» (deployment_threat_model.md §3.1) فالإصلاح يجب أن يكون فيها
لا في المستدعين.

**لماذا S2 وليست S4** — الأصل رقم 2 في نموذج التهديد («الملفات السرية
داخل المشروع… يجب ألا تُقرأ أو تُسرّب إلى سياق النموذج») يُخترَق
**داخل** عقد localhost لا خارجه: الوكيل يُوجَّه بمحتوى المشروع نفسه
(أسماء ملفات تصل السياق عبر `tool_list_dir`/الفهرس)، فمستودع مُعَدّ
أو نموذج مُضلَّل يقرأ السر ويضعه في سياق يُرسل للمزود. لا يتطلب
مهاجمًا شبكيًا — ولهذا لا يُعفى بحدّ «أحادي المستخدم».
**الأثر على Windows أشدّ**: NTFS يقلّم المسافات/النقاط اللاحقة، فـ
`.env ` و`.env` **الملف نفسه** — أي أن قراءة السر الحقيقي تنجح بلا
أي تحضير للمستودع (غلاف سطح المكتب Windows-أولًا — TSK-727).

> ### ⚠️ تصحيح ذاتي جوهري (TSK-CEV-117، 2026-08-03) — CEV-R3
> أعدتُ التحقق من هذه البطاقة قبل الإصلاح فوجدتُ **ادعاءً مبالغًا**
> أعلاه يجب تصويبه صراحةً (النص الأصلي محفوظ append-only):
> - **الخطأ**: سطر الدليل `read('.env ') -> API_KEY=…` وُصف بأنه
>   «**تسريب فعلي**» لسر حقيقي على البيئة الحالية. هذا **غير صحيح على
>   POSIX**: أُثبت حيًّا أن `resolve_workspace_path(root, '.env ')`
>   يُحلّ إلى ملفٍ **مختلف** `'/…/proj/.env '` بـ`exists=False` —
>   أي لا محتوى يُقرأ. ما أُثبت فعلًا: الاسم **يمر من البوابة** (لا
>   يُصنَّف سرًّا)، وهو إفلاتُ حجب لا قراءةَ سر.
> - **الحدّ الصحيح**: (أ) على POSIX الأثر = عضو من **عائلة** قائمة
>   الحجب يفلت (ملف أُنشئ بذلك الاسم: مستودع مُعَدّ، مخرَج أداة،
>   استخراج أرشيف) ⇒ يدخل الفهرس/السياق؛ (ب) على **Win32** — وهي
>   منصة الشحن الأولى (TSK-727) — النظام يقلّم المسافات/النقاط
>   اللاحقة فيصير `.env ` **نفس** `.env` ⇒ قراءة السر الحقيقي.
> - **الشدة تبقى S2** لكن **بمبرر مصحَّح**: ليست «تسريبًا مؤكَّدًا على
>   كل منصة» بل **اختراق دفاعٍ مركزي، مُستغَلّ كليًا على Win32
>   ومُستغَلّ جزئيًا على POSIX**. لا تهوين ولا تهويل.
> - **متجه لم يُرصد في الجولة الأصلية** (يُقيَّد الآن): **NTFS ADS** —
>   `.env::$DATA` و`.env:$DATA` كانا يمرّان أيضًا، وهما على Win32
>   يفتحان **نفس تيار بيانات** الملف الأصلي. كذلك `.env . . `
>   (نقاط/مسافات متناوبة) و`.env\u200b` (ZWSP لا يراه `str.strip`).
> - **خطأ ثانٍ في الجدول أعلاه**: `.env.` وُصف بأنه «يمر ❌» — التحقق
>   أظهر أنه كان **محجوبًا بالأصل** (يلتقطه فرع `startswith('.env.')`).
>   الصفوف الصحيحة للتجاوز: `.env `، `id_rsa `/`id_rsa.`، `cert.pem `.
>
> **الحالة: ✅ مُغلقة بـTSK-CEV-117** (تطبيع مركزي في
> `normalize_secret_name` + 290 اختبار انحدار). راجع تقرير TSK في
> MASTER_REVIEW.

**ليست حجبًا للبطاقة بحكم ذاتي**: S2 مفتوحة ⇒ §6 ينص «S1/S2 مفتوحة =
البطاقة محجوبة». التصنيف مرفوع للمالك مع خيار الإصلاح (سطر تطبيع
واحد + اختبارات) — القرار له لا للمدقق.

**الإصلاح المقترح (لم يُنفَّذ — خارج تفويض G7 «قراءة-فقط»)**:
تطبيع `name_lower = path.name.strip().rstrip('.').lower()` (أو
`.rstrip(' .\t')`) قبل الفروع الثلاثة + اختبار انحدار لكل فئة
(CEV-R9: اختبار يعيد إنتاج التسريب أعلاه).

### CEV-F-017 — تحديث اتساع (G7 Red Team، نفس الجولة)
**C1 / S4 — توسيع نطاق مُثبت.** الفجوة الصرفية ليست صيغة واحدة
(«تعيد») بل **محور تصريف كامل**: أُثبت حيًّا أن `يعيد كتابة` و
`نعيد تصميم` لا يُلتقطان أيضًا بأنماط `_COMPLEX_REQUEST_PATTERNS`
(chain/orchestrator.py:110-121)، بينما `أعد هيكلة` و`إعادة هيكلة` و
`refactor` تُلتقط (ضوابط خضراء). النمط المقترح في FI-17 وُسِّع
وتُحقق منه: `[أاتنيو]ع(?:د|يد\w{0,2})\s*.{0,6}(?:هيكل|كتابة|تصميم)`
يلتقط الخمس صيغ كلها. يبقى S4 (جودة توجيه؛ مسار direct يمر بنفس
ApprovalGate) — ولا يحجب البطاقة.

### CEV-F-006 — تحديث كمّي (TSK-CEV-117، 2026-08-03): قياس A/B يُثبّت بيئية رجفة `search_perf`
**C1 / S4 — لا فعل مطلوب؛ التحديث لإغلاق شكٍّ نشأ أثناء الإصلاح.**
أثناء TSK-CEV-117 سقط `test_tool_search_code_path_under_1s` (1.006s)
في تشغيلة `check.sh` **بعد** تعديلي، فكان الاحتمال الجدّي أن يكون
**انحدار أداء سببتُه أنا** (التطبيع يضيف عملًا لكل ملف). لم أفترض
البيئة — قِستُ:

**قياس A/B، 6 تشغيلات لكل ذراع، نفس الجلسة والعتاد** (استُخدم نسخ
ملف من/إلى `/tmp` بدل `git stash` تفاديًا لخطر فقدان عمل غير مكوميت):
| الذراع | النتيجة |
|---|---|
| **الأساس** (`path_policy` قبل الإصلاح @ 8a99cf5) | **3 فشل / 6** — 1.03s، 1.08s، 1.18s |
| **مع الإصلاح** (@ 97a1a48) | **0 فشل / 6** — مرّت ستًّا |

**الاستنتاج (يخالف الفرض الأول)**: الرجفة **سابقة للإصلاح** وأشدّ
على الأساس. والإصلاح — بعد إضافة المسار السريع `_needs_normalization`
وتقليل نداءات التطبيع في حلقة `path.parts` — صار **أقل عرضة** للرجفة
لا أكثر.

**قياس الكلفة الحقيقية** (`tool_search_code` على 5000 ملف، وسيط
5 تشغيلات): **0.120s (أساس) → 0.131s (مُصلَح) = +11ms (~9%)** أي أقل
من عتبة 1s بنحو **7×**. القياس المعزول للدالة: 1.50 µs/مسار → 3.95،
لكن أثره على الزمن الكلي هامشي لأن الغالب هو قراءة الملفات.

**مسار التحسين الموثَّق (شفافية)**: النسخة الساذجة للإصلاح كانت
22.9ms/5000 (3× الأساس)، ونسخة أولى من المسار السريع كانت **أسوأ**
(60.4ms) بسبب `str.isspace()` + `any(generator)`؛ الاستقرار على
`frozenset.isdisjoint` أعطى 0.36 µs/اسم مقابل 1.90 (أسرع ~5×).

**لا تغيير في تصنيف CEV-F-006** (رجفات عتبات توقيت على 2 vCPU) ولا
في العتبة نفسها: **لم تُرفع العتبة ولم يُشطب الاختبار ولم يُوسم
`xfail`** — الحل كان تحسين الكود الحقيقي.

### CEV-F-006 — تصحيح توصيف جوهري (TSK-CEV-117): الرجفة **متكررة ومنهجية** لا «مرة واحدة»
**C1 / S3 (رُفعت من S4) — قياس مضبوط، 9 تشغيلات كاملة.**
التوصيف الأصلي أعلاه (:395-404) وصف `test_search_perf` بأنه فشل
«**مرة واحدة غير متكررة**» و«يمر معزولًا ×3». القياس الآن **يخالف
ذلك** ويجب تصحيحه (append-only — النص الأصلي باقٍ):

**الذراع أ — الأساس بلا أي تعديل مني** (`path_policy` @ 8a99cf5،
مع `--ignore` لملف الاختبار الجديد كي يكون الذراع صالحًا):
`1 failed, 2285 passed` — **3 من 3 تشغيلات** (90.93s / 89.56s / 90.71s).
الفاشل في كل مرة هو `test_tool_search_code_path_under_1s`.

**الذراع ب — مع إصلاح TSK-CEV-117**: **2 فشل من 3** (واحدة خضراء
تمامًا: `2576 passed, 34 skipped`).

**منفردًا** (بلا حمل الحزمة): الأساس 3/6 فشل؛ المُصلَح 1/6 فشل.

**الاستنتاجات (تصحيح مزدوج لروايتَي الجلسة السابقة والحالية)**:
1. الرجفة **ليست عابرة** بل **منهجية تحت حمل الحزمة** على 2 vCPU —
   لذا رُفعت الشدة S4 → **S3** (تلوّث بوابة `check.sh`: `rc=1` دائم
   تقريبًا يُفقد البوابة قيمتها كإشارة).
2. **إصلاحي ليس السبب** — الأساس أسوأ (3/3 مقابل 2/3). كان فرضي
   الأول أنني سببتُ انحدارًا؛ القياس نفاه.
3. **ليست «تمر معزولًا ×3» كما وُثِّق** — تفشل معزولةً أيضًا بنسبة
   1–3 من 6.

**لم يُتخذ أي مسكّن**: العتبة لم تُرفع، الاختبار لم يُشطب ولا وُسم
`xfail`، والمعامل الحقيقي المقيس لكلفة الإصلاح +11ms على ~130ms
(الاختبار يقيس زمنًا كليًا يتجاوز 1s فقط عند مزاحمة CPU).
**التوصية (قرار مالك — خارج نطاق TSK-CEV-117)**: تقسية الاختبار
بأخذ **أفضل 3 قياسات** أو رفع العتبة إلى ما يناسب 2 vCPU مع تثبيت
معيار نسبي، في TSK منفصلة. لم أُنفِّذها ذاتيًا لأنها تغيير عقد
اختبار أداء لا يشمله تفويض «إصلاح F-018».

### دفعة الإقفال الشامل BATCH-CLOSEOUT (D-18، 2026-08-04) — حالات الإغلاق
**بموجب قرار مالك D-18** («متخلش حاجه وراك مفتوحه» — تفويض إغلاق كل
البنود المعروضة في تقرير الاستئناف S110). سجل الإغلاق append-only:

- **CEV-F-017 ✅ مُغلقة (TSK-CEV-118)**: محور المضارعة وُسّع كاملًا في
  `_COMPLEX_REQUEST_PATTERNS` (orchestrator.py:120-122) — فئة الحرف
  الأول `[أاتني]` + جذور بلا تاء المصدر (هيكل/كتاب/تصميم) تلتقط
  الضمائر المتصلة. تحقق حي: مجس G7 الأصلي («…وتعيد هيكلته كله…»)
  يصل `full_chain` الآن. 5 اختبارات انحدار جديدة
  (`TestLexiconGapF017` في test_routing_matrix.py) منها ضابط سلبي
  («اكتب لي دالة» تبقى direct — لا انزلاق للمعجم). FI-17 أُغلقت معها.
- **CEV-F-006 ✅ مُقسّاة (TSK-CEV-119)**: `test_search_perf` تحوّل إلى
  **أفضل-من-3** (`_best_of` — الخيار الأول في توصية التصحيح أعلاه):
  العتبة 1.0s **لم تُرفع**، الاختبار لم يُشطب ولا وُسم xfail؛
  انحدار خوارزمي حقيقي يفشل الثلاث تشغيلات معًا بينما ضجيج جدولة
  2 vCPU يسقط (يكفي أن تسلم واحدة). الشدة تهبط S3 ⇒ **مُدارة** (تلوث
  بوابة check.sh الذي رفعها إلى S3 زال بالتقسية).
- **providers module_from_spec ✅ (D-18 يقيّد استثناء §0.8 تفويضًا
  موضعيًا)**: حارس None بنمط genspark.py:58-59 أُضيف نصًّا في
  you_com/perplexity/blackbox، ورُفعت استبعاداتها الثلاثة من بوابة
  mypy في check.sh (يبقى openai_shelby مستثنى — نص D-18). حارسا
  انحدار جديدان في test_mypy_gate_614: الاستبعادات المرفوعة لا تعود
  + القبول المادي للحارس في الملفات الأربعة. `mypy providers/` =
  Success (13 ملفًا).
- **CEV-F-011 ✅ مُغلقة (TSK-CEV-120)**: الـ43 تشخيصًا أُزيلت ميكانيكيًا
  (استيرادات/متغير over/بادئات f زائدة — صفر سلوك)، والاستثناءان
  الموثقان (chain/router.py:28 TYPE_CHECKING + context/engine.py:37
  re-export مقصود) وُسما `noqa: F401`. البوابة الجديدة:
  `flake8 --select=F` في check.sh (تحترم noqa) + `flake8>=7,<8` في
  requirements-dev. FI-08 أُغلقت معها. تصحيح عدّ: القياس الحي وجد
  **43** تشخيصًا لا 42 كما وثّق الاكتشاف الأصلي.
- **CEV-F-009 ✅ مُغلقة (TSK-CEV-121)**: `_err_status` في routes/files.py
  يوحّد العقد الرقمي: PermissionError⇒403، FileNotFoundError⇒404،
  ValueError⇒400، غيرها⇒500 — في المعالجات الست ذات الـ500 الشامل
  ومسار القراءة (كان 404 شاملًا). fail-closed بلا تغيير. 4 اختبارات
  عقد (منها مجس G6 الأصلي: `POST ..%2fpwned.txt` ⇒ 403 ولا ملف يُكتب).
- **CEV-F-008 ✅ مُغلقة (TSK-CEV-122)**: 19 إعلان radius خام + صف
  `8px 8px 0 0` استُبدلت بـ`var(--radius)`/`var(--radius-lg)`؛ توكن
  جديد `--duration-fast: 0.15s` في tokens.css يستهلكه 10 مواضع
  (كانت الطبقة الثالثة الخام). عقد الألوان (الوحيد المفروض CI) كان
  سليمًا سلفًا ولم يُمس.
- **CEV-F-007 ✅ مُغلقة (TSK-CEV-123)**: `static/icons/favicon.svg`
  (رمز ∞ من هوية INFINICODE بلون --blue الداكن #6ea1ff — لا أصول
  خارجية، CEV-R11) + `<link rel="icon">` في index.html — يغلق 404
  الوحيد في التحميل الحي.
- **FI-02 ✖ WAIVED BY OWNER (D-18 نصًّا)**: تفكيك server.py لا يُنفَّذ
  ولا يبقى بندًا مفتوحًا — موسومة في FUTURE_IMPROVEMENTS.
- **إصلاح جانبي مقيَّد**: حارس PM-01 في test_instrumentation_609 كان
  يفحص وجود CharsPerTokenEstimator في server.py — المستهلك الفعلي
  في core/chat_dispatch.py منذ TSK-612 واستيراد server كان ميتًا
  (أزالته TSK-CEV-120)؛ الحارس صُوّب ليفحص الوحدة المالكة للمنطق.
