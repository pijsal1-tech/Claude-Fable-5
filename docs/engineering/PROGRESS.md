...
- **User directive**: ممنوع git commit/push/PR — المستخدم يرفع يدويًا.
  كل التغييرات في working tree فقط فوق ac43f6c.
- **TSK-201** ✅ Completed — `server.py`: كتلتا `apply_all_actions`/
  `execute_plan` المتطابقتان (سابقًا L1862–1925) دُمجتا في `_apply_batch`؛
  السلوك مُثبَّت بـ golden التُقط من الكود قبل التعديل
  (`tests/goldens/apply_batch_frames.json`) — مطابقة بايت-بايت.
- **TSK-101** ✅ Completed — `actions/response_parser.py`: توقيع
  ...
  mode=None (chain/action_applier) = سلوك تاريخي كامل.
- **TSK-102** ✅ Completed — بلوكات bash في الـ fallback لا تُنتج أوامر
  إلا بوسم سطري صريح `CMD:`؛ `rm -rf build/` كمثال شرح لم يعد يصبح
  CommandBlock. بلوك ```CMD الصريح بلا تغيير.
- **QA-T05 gate** ✅ — `tests/unit/test_parser_mode_awareness.py` (11 اختبارًا):
  3 ردود مزيفة (منها rm -rf) → صفر actions في chat؛ بقية الأوضاع بلا
  تغيير؛ صفر نداءات AI خارجية.
- **QA-T08 seed** ✅ — `tests/integration/test_apply_batch_golden.py`
  (golden/parity/backup-flag — 3 اختبارات).
- **Full suite**: 5 failed / 1490 passed / 63 skipped — الإخفاقات الخمسة
  موجودة مسبقًا على HEAD النظيف (تحقق سابقًا عبر git worktree منفصل):
  test_file_icons / test_history_consumers / test_rollback_ui /
  test_symbol_index / test_theme_tokens — خارج نطاق M1، لم تُلمس.
- **Files changed (للرفع اليدوي)**:
  1. `server.py` — مُعدّل (TSK-201 + TSK-101)
  2. `actions/response_parser.py` — مُعدّل (TSK-101 + TSK-102)
  3. `tests/unit/test_parser_mode_awareness.py` — جديد (QA-T05)
  4. `tests/integration/test_apply_batch_golden.py` — جديد (QA-T08 seed)
  5. `tests/goldens/apply_batch_frames.json` — جديد (golden قبل الدمج)
...
- **EXACT RESUME POINT**: TSK-103 (Current Task) — توحيد مسارات حقن
  السياق تحت ContextBudget (BUG-03): server.py مسار الملف المكتشف
  (كان L1332–1339) + مسار attach المجلد (كان L1782–1791) ← تمرير
  المحتوى كمصدر إلى gather_message_context بدل الإلحاق الخام
  بـ user_text؛ السقف من config.yaml:context_budget؛ القبول: مجلد 15
  ملفًا + ملف 100KB → الحمولة ≤ السقف (QA-T06). بعدها: TSK-104 ثم
  TSK-105؛ بوابة QA-T06/T07 تُغلق M1.
