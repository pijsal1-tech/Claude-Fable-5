...
| Field | Value |
|---|---|
| last-updated | 2026-08-04 (Session 112 تكملة 7 — **القرار 8 (ACP) مُخطَّط: مواصفة TSK-736 مكتوبة (عميل ACP فقط؛ كل كتابة خلف ApprovalGate + كل قراءة عبر denylist؛ chain/acp/ — صفر مساس بـ providers/)**؛ **القرارات 2–7 من تسلسل D-19 مُغلقة 🏁**: (7) TSK-735 مزودو API-key القياسيون بموجب تعديل D-20 (SECTION 0.8 موضعي كسابقة D-18) — core/provider_keys.py (ملف جانبي مُتجاهَل + SECRETS_DENYLIST) + providers/openai_compat.py (مزود عام واحد /chat/completions + SSE، المفتاح لا يُردَّد أبدًا — كناري) + توصيل server (/api/models براية key_configured + فرع تبديل عام بقراءة مفتاح طازجة) + 50 اختبارًا جديدًا؛ سياق تاريخي: (2) v1.0.0؛ (3) mypy 100%؛ (4) TSK-732؛ (5) TSK-733؛ (6) TSK-734؛ البوابة ALL GREEN rc=0: 2729P/34S/0F) |
...
| current-phase | **تنفيذ تسلسل D-19**: القرارات 2 (v1.0.0) 🏁 + 3 (mypy 100%) 🏁 + 4 (TSK-732) 🏁 + 5 (TSK-733) 🏁 + 6 (TSK-734) 🏁 + 7 (TSK-735 مزودو API-key بموجب D-20) 🏁 → **التالي: القرار 8 (ACP)** → 9 (network exposure أخيرًا — يعيد فحص مسار permissions_overrides.json **و** provider_keys.json)؛ 10/11 مستقلان في أي وقت |
| current-task | **القرار 8 من تسلسل D-19 (ACP) — التخطيط مُنجَز، التنفيذ لم يبدأ**: مواصفة TSK-736 مكتوبة (المحرر عميل ACP فقط؛ 7 قرارات واعية — كل كتابة خلف ApprovalGate نفسها + كل قراءة عبر path_policy/denylist + opt-in عبر acp.agents + صفر مساس بـ providers/؛ 3 شرائح 736a→b→c — DEVELOPMENT_TASKS §TSK-736)؛ لا تعديل SECTION 0.8 جديد (الكود في chain/acp/). **التالي: تنفيذ 736a** (نواة JSON-RPC النقية) |
