...
| Field | Value |
|---|---|
| last-updated | 2026-08-05 (Session 112 تكملة 9 — **القرار 9 من تسلسل D-19 (الأخير): التخطيط مُنجَز** — نموذج تهديد كامل (T1–T6) + مواصفة TSK-737 في DEVELOPMENT_TASKS EOF: حارس إقلاع fail-closed يرفض أي ربط غير loopback إلا بـ --unsafe-expose-network، وحتى تحتها تُقفَل المسارات الثلاثة المرحَّلة (POST /api/permissions=403 + acp_prompt مرفوض + provider_keys بلا كتابة أصلًا)؛ الحكم الحاسم T4: قناة الموافقة WS غير مُصادَقة ⇒ لا قيمة لأي تقسية سلوكية دون منع الربط المكشوف؛ البوابة ALL GREEN rc=0: 2813P/34S/0F) |
...
| current-phase | **تنفيذ تسلسل D-19**: القرارات 2–8 مُغلقة 🏁 → **الجاري: القرار 9 (network exposure — الأخير) = TSK-737**: التخطيط D-7 مُنجَز (نموذج التهديد أعاد فحص المسارات الثلاثة المرحَّلة من TSK-734/735/736)؛ الشرائح: 737a (core/network_guard.py النقية) → 737b (توصيل الحارس server/meta) → 737c (إغلاق التوثيق وإقفال التسلسل)؛ 10/11 مستقلان في أي وقت |
| current-task | **TSK-737 (القرار 9) — التالي: تنفيذ 737a**: core/network_guard.py (is_loopback_host — 127/8 + ::1 + localhost حرفيًا؛ fail-closed: ما لم يثبت loopback فهو مكشوف + exposure_refusal_message) + اختبارات وحدة بارامترية — صفر شبكة/ربط فعلي في الاختبارات |
