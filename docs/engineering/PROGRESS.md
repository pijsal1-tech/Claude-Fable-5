...
| Field | Value |
|---|---|
| last-updated | 2026-08-05 (Session 112 تكملة 8 — **القرار 8 من تسلسل D-19 مُغلق 🏁: TSK-736 ACP** — المحرر عميل ACP لوكلاء خارجيين governed: chain/acp/ (protocol + connection + agent_process + governed_fs — كل fs/write وrequest_permission عبر نفس ApprovalGate، كل fs/read عبر path_policy/denylist، صفر مساس بـ providers/) + توصيل server (GET /api/acp/agents ببادئة 🤝 بلا كشف command/args + نوع WS acp_prompt + بث acp_update) + وكيل دمية محلي و8 اختبارات تكامل (+84 اختبارًا إجمالًا)؛ **القرارات 2–8 من تسلسل D-19 مُغلقة 🏁**؛ البوابة ALL GREEN rc=0: 2813P/34S/0F) |
...
| current-phase | **تنفيذ تسلسل D-19**: القرارات 2 (v1.0.0) 🏁 + 3 (mypy 100%) 🏁 + 4 (TSK-732) 🏁 + 5 (TSK-733) 🏁 + 6 (TSK-734) 🏁 + 7 (TSK-735) 🏁 + 8 (TSK-736 ACP) 🏁 → **التالي: القرار 9 (network exposure — الأخير)**: يعيد فحص مسار permissions_overrides.json **و** provider_keys.json **و** مسارَي ACP (الكتابة/الإذن) قبل أي تعريض شبكي؛ 10/11 مستقلان في أي وقت |
| current-task | **القرار 8 مُغلق 🏁 — بانتظار الشروع في القرار 9 (network exposure)**: التسلسل D-19 وصل لآخر قراره؛ القرار 9 يتطلب نموذج تهديد كامل يعيد فحص: (أ) مسار تحرير permissions_overrides.json (TSK-734)، (ب) مسار provider_keys.json (TSK-735)، (جـ) مسارَي كتابة/إذن ACP (TSK-736) — كلها صُممت لـ localhost فقط |
