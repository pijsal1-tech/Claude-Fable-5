...
| current-task | **لا مهمة جارية — D-19 مستوفى بكل بنوده 🏁**: بانتظار توجيه المالك؛ بند متابعة وحيد غير حاجب: نتيجة تشغيل `scripts/perf_probe.py` على جهاز المالك الحقيقي (القرار 11) — تُلحق عند ورودها وقد تفتح TSK إصلاح إن ظهرت الرجفة (الخيارات الأخرى: بنود FI، إغلاقات EOP) |
