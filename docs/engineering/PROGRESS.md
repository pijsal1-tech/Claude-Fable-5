...
| current-phase | **تنفيذ تسلسل D-19**: القرارات 2 (v1.0.0) 🏁 + 3 (mypy 100%) 🏁 + 4 (TSK-732) 🏁 + 5 (TSK-733) 🏁 + 6 (TSK-734 تحرير الأذونات) 🏁 → **التالي: القرار 7** (مزودو API-key — تعديل SECTION 0.8) → ثم 8 (ACP) → 9 (network exposure أخيرًا)؛ 10/11 مستقلان في أي وقت |
| current-task | **القرار 7 من تسلسل D-19 — لم يبدأ**: تخطيط أولًا (إجراء D-7 الدائم): مزودو API-key يتطلبون تعديل SECTION 0.8 من الدستور — صياغة التعديل ثم خطة TSK |
